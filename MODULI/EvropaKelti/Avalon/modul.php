 ...
 *
 * 📰 NAMEN:
 *     Avalon — keltska mistična tradicija arthurijanske legende. Otok jabolk, vitezi in sveti gral.
 *
 * 🔧 JAVNE FUNKCIJE:
 *     - modul_avalon_akcija(string $akcija, array $podatki): array
 *
 * 📡 ODVISNOSTI:
...
function _modul_avalon_info(array $podatki): array {
    $uporabnik = Modul_Bridge::uporabnik_pridobi();
    return odziv_uspeh([
        'ime'       => 'Avalon',
        'id'        => 'avalon',
        'verzija'   => '1.0.0',
        'opis'      => 'Avalon — keltska mistična tradicija arthurijanske legende. Otok jabolk, vitezi in sveti gral.',
        'uporabnik' => $uporabnik['ime'] ?? 'Gost',
    ], 'Informacije o modulu');
}

function _modul_avalon_domov(array $podatki): array {
    return odziv_uspeh([
        'sporocilo' => 'Pozdravljen v modulu Avalon!',
        'cas'       => time(),
    ], 'Domov');
}
...
if (basename($_SERVER['SCRIPT_FILENAME'] ?? '') === 'modul.php' && !defined('SISTEM_OBSTAJA')) {
    ...
    $podatki = $_REQUEST;
    $odziv   = modul_avalon_akcija($akcija, $podatki);
