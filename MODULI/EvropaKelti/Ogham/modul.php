 ...
 *
 * 📰 NAMEN:
 *     Ogham — starodavna keltska pisava in orakelj dreves. Vsakemu drevesu svoja runa, vsaki runi svoja moč.
 *
 * 🔧 JAVNE FUNKCIJE:
 *     - modul_ogham_akcija(string $akcija, array $podatki): array
 *
 * 📡 ODVISNOSTI:
...
function _modul_ogham_info(array $podatki): array {
    $uporabnik = Modul_Bridge::uporabnik_pridobi();
    return odziv_uspeh([
        'ime'       => 'Ogham',
        'id'        => 'ogham',
        'verzija'   => '1.0.0',
        'opis'      => 'Ogham — starodavna keltska pisava in orakelj dreves. Vsakemu drevesu svoja runa, vsaki runi svoja moč.',
        'uporabnik' => $uporabnik['ime'] ?? 'Gost',
    ], 'Informacije o modulu');
}

function _modul_ogham_domov(array $podatki): array {
    return odziv_uspeh([
        'sporocilo' => 'Pozdravljen v modulu Ogham!',
        'cas'       => time(),
    ], 'Domov');
}
...
if (basename($_SERVER['SCRIPT_FILENAME'] ?? '') === 'modul.php' && !defined('SISTEM_OBSTAJA')) {
    ...
    $podatki = $_REQUEST;
    $odziv   = modul_ogham_akcija($akcija, $podatki);
