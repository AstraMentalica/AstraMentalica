 ...
 *
 * 📰 NAMEN:
 *     Druidica — modrost druidov. Sveta drevesa, naravni cikli, keltski obredi in kozmologija.
 *
 * 🔧 JAVNE FUNKCIJE:
 *     - modul_druidica_akcija(string $akcija, array $podatki): array
 *
 * 📡 ODVISNOSTI:
...
function _modul_druidica_info(array $podatki): array {
    $uporabnik = Modul_Bridge::uporabnik_pridobi();
    return odziv_uspeh([
        'ime'       => 'Druidica',
        'id'        => 'druidica',
        'verzija'   => '1.0.0',
        'opis'      => 'Druidica — modrost druidov. Sveta drevesa, naravni cikli, keltski obredi in kozmologija.',
        'uporabnik' => $uporabnik['ime'] ?? 'Gost',
    ], 'Informacije o modulu');
}

function _modul_druidica_domov(array $podatki): array {
    return odziv_uspeh([
        'sporocilo' => 'Pozdravljen v modulu Druidica!',
        'cas'       => time(),
    ], 'Domov');
}
...
if (basename($_SERVER['SCRIPT_FILENAME'] ?? '') === 'modul.php' && !defined('SISTEM_OBSTAJA')) {
    ...
    $podatki = $_REQUEST;
    $odziv   = modul_druidica_akcija($akcija, $podatki);
