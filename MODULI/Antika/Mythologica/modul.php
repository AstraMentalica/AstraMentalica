 ...
 * ============================================================
 * MODUL: Mythologica
 * POT: MODULI/Antika/Mythologica/modul.php
 ...
 *
 * 🔧 JAVNE FUNKCIJE:
 *     - modul_mythologica_akcija(string $akcija, array $podatki): array
 *
 * 📡 ODVISNOSTI:
...
function modul_mythologica_akcija(string $akcija, array $podatki = []): array {
    if (!Modul_Bridge::vloga_preveri('S0')) {
        return odziv_napaka('Dostop zavrnjen', 403);
    }

    return match($akcija) {
        'info'  => _modul_mythologica_info($podatki),
        'domov' => _modul_mythologica_domov($podatki),
...
function _modul_mythologica_info(array $podatki): array {
    $uporabnik = Modul_Bridge::uporabnik_pridobi();
    return odziv_uspeh([
        'ime'       => 'Mythologica',
        'id'        => 'mythologica',
        'verzija'   => '1.0.0',
        'opis'      => 'Mythologica — zbirka mitov in legend antičnega sveta. Heroji, pošasti in kozmogonske zgodbe.',
        'uporabnik' => $uporabnik['ime'] ?? 'Gost',
    ], 'Informacije o modulu');
}

function _modul_mythologica_domov(array $podatki): array {
    return odziv_uspeh([
        'sporocilo' => 'Pozdravljen v modulu Mythologica!',
        'cas'       => time(),
    ], 'Domov');
}
...
if (basename($_SERVER['SCRIPT_FILENAME'] ?? '') === 'modul.php' && !defined('SISTEM_OBSTAJA')) {
    ...
    $podatki = $_REQUEST;
    $odziv   = modul_mythologica_akcija($akcija, $podatki);
