...
require_once __DIR__ . '/modul_mythologica_funkcije.php';
require_once __DIR__ . '/modul_mythologica_pravila.php';
require_once __DIR__ . '/modul_mythologica_jsonbaza.php';

class ModulMythologica {
    private $db;
    private $config;

    public function __construct() {
        $this->db = new ModulMythologicaJsonBaza();
        $this->config = $this->db->pridobiVse();
    }

    public function pridobiOsnovnePodatke(): array {
        return [
            'ime'            => $this->config['ime'] ?? 'Mythologica',
    ...
    public function pridobiVsebino(array $param = []): array {
        $html = "<div class='modul-mythologica'>" . htmlspecialchars($this->config['opis'] ?? '') . "</div>";
        return ['naslov' => $this->config['opis'] ?? 'Mythologica', 'vsebina' => $html];
    }
