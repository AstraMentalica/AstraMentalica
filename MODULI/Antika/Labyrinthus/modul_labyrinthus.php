...
require_once __DIR__ . '/modul_labyrinthus_funkcije.php';
require_once __DIR__ . '/modul_labyrinthus_pravila.php';
require_once __DIR__ . '/modul_labyrinthus_jsonbaza.php';

class ModulLabyrinthus {
    private $db;
    private $config;

    public function __construct() {
        $this->db = new ModulLabyrinthusJsonBaza();
        $this->config = $this->db->pridobiVse();
    }

    public function pridobiOsnovnePodatke(): array {
        return [
            'ime'            => $this->config['ime'] ?? 'Labyrinthus',
    ...
    public function pridobiVsebino(array $param = []): array {
        $html = "<div class='modul-labyrinthus'>" . htmlspecialchars($this->config['opis'] ?? '') . "</div>";
        return ['naslov' => $this->config['opis'] ?? 'Labyrinthus', 'vsebina' => $html];
    }
