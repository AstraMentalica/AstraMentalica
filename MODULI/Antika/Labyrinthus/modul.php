 ...
 * ============================================================
 * MODUL: Labyrinthus
 * POT: MODULI/Antika/Labyrinthus/modul.php
 ...
 *
 * 🔧 JAVNE FUNKCIJE:
 *     - modul_labyrinthus_akcija(string $akcija, array $podatki): array
 *
 * 📡 ODVISNOSTI:
...
function modul_labyrinthus_akcija(string $akcija, array $podatki = []): array {
    if (!Modul_Bridge::vloga_preveri('S0')) {
        return odziv_napaka('Dostop zavrnjen', 403);
    }

    return match($akcija) {
        'info'  => _modul_labyrinthus_info($podatki),
        'domov' => _modul_labyrinthus_domov($podatki),
...
function _modul_labyrinthus_info(array $podatki): array {
    $uporabnik = Modul_Bridge::uporabnik_pridobi();
    return odziv_uspeh([
        'ime'       => 'Labyrinthus',
        'id'        => 'labyrinthus',
        'verzija'   => '1.0.0',
        'opis'      => 'Labyrinthus — skrivnosti antičnih labirintov in iniciacijskih poti. Simbolika, miti in duhovni pomen.',
        'uporabnik' => $uporabnik['ime'] ?? 'Gost',
    ], 'Informacije o modulu');
}

function _modul_labyrinthus_domov(array $podatki): array {
    return odziv_uspeh([
        'sporocilo' => 'Pozdravljen v modulu Labyrinthus!',
        'cas'       => time(),
    ], 'Domov');
}
...
if (basename($_SERVER['SCRIPT_FILENAME'] ?? '') === 'modul.php' && !defined('SISTEM_OBSTAJA')) {
    ...
    $podatki = $_REQUEST;
    $odziv   = modul_labyrinthus_akcija($akcija, $podatki);
