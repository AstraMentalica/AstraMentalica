...
class ModulCorpusMysticum {
    ...
    public function __construct() {
        $this->db = new ModulCorpusMysticumJsonBaza();
        $this->config = $this->db->pridobiVse();
    }

    public function pridobiOsnovnePodatke() {
        return [
            'ime' => $this->config['ime'] ?? 'CorpusMysticum',
    ...
    public function pridobiVsebino($param = []) {
        $html = \"<div class='modul-corpusmysticum'>\" . htmlspecialchars($this->config['opis'] ?? '') . \"</div>\";
        return ['naslov' => $this->config['opis'] ?? 'CorpusMysticum', 'vsebina' => $html];
    }
