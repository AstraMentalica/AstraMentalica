 ...
 *
 * 📰 NAMEN:
 *     Svet sanj — japonska tradicija sanjske interpretacije in yume no michi.
 *
 * 🔧 JAVNE FUNKCIJE:
 *     - modul_yume_akcija(string $akcija, array $podatki): array
 *
 * 📡 ODVISNOSTI:
 *     - Modul_Bridge
 *
...
function _modul_yume_info(array $podatki): array {
    ...
    return odziv_uspeh([
        ...
        'kategorija'=> 'Japonska',
        'verzija'   => '1.0.0',
        'opis'      => 'Svet sanj — japonska tradicija sanjske interpretacije in yume no michi.',
        'uporabnik' => $uporabnik['ime'] ?? 'Gost',
    ], 'Informacije o modulu');
}

function _modul_yume_domov(array $podatki): array {
    return odziv_uspeh([
        'sporocilo' => 'Pozdravljen v modulu Yume!',
        'cas'       => time(),
    ], 'Domov');
}
...
if (basename($_SERVER['SCRIPT_FILENAME'] ?? '') === 'modul.php' && !defined('SISTEM_OBSTAJA')) {
    ...
    $odziv   = modul_yume_akcija($akcija, $_REQUEST);
