 ...
 *
 * 📰 NAMEN:
 *     Nordijska magija — seidr, galdr, nordijska šamanistična tradicija.
 *
 * 🔧 JAVNE FUNKCIJE:
 *     - modul_seidr_akcija(string $akcija, array $podatki): array
 *
 * 📡 ODVISNOSTI:
 *     - Modul_Bridge
 *
...
function _modul_seidr_info(array $podatki): array {
    ...
    return odziv_uspeh([
        ...
        'kategorija'=> 'EvropaNordija',
        'verzija'   => '1.0.0',
        'opis'      => 'Nordijska magija — seidr, galdr, nordijska šamanistična tradicija.',
        'uporabnik' => $uporabnik['ime'] ?? 'Gost',
    ], 'Informacije o modulu');
}

function _modul_seidr_domov(array $podatki): array {
    return odziv_uspeh([
        'sporocilo' => 'Pozdravljen v modulu Seidr!',
        'cas'       => time(),
    ], 'Domov');
}
...
if (basename($_SERVER['SCRIPT_FILENAME'] ?? '') === 'modul.php' && !defined('SISTEM_OBSTAJA')) {
    ...
    $odziv   = modul_seidr_akcija($akcija, $_REQUEST);
