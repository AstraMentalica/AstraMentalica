...
class ModulAfricanaMystica {
    ...
    public function __construct() {
        $this->db = new ModulAfricanaMysticaJsonBaza();
        $this->config = $this->db->pridobiVse();
    }

    public function pridobiOsnovnePodatke(): array {
        return [
            'ime'            => $this->config['ime'] ?? 'AfricanaMystica',
    ...
    public function pridobiVsebino(array $param = []): array {
        $html = "<div class='modul-africanamystica'>" . htmlspecialchars($this->config['opis'] ?? '') . "</div>";
        return ['naslov' => $this->config['opis'] ?? 'AfricanaMystica', 'vsebina' => $html];
    }
