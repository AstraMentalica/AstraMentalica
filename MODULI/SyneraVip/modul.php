...
/**
 * MODUL: SyneraVip
 * POT: MODULI/SyneraVip/modul.php
 * VERZIJA: 1.0.0 (24.6.2026)
 *
 * 🏛️ NIVO: MODUL
 *
 * 📰 NAMEN:
 *     Standardna vstopna točka za SyneraVip modul
 *     Avtomatsko generirano
 *
 * 🔧 JAVNE FUNKCIJE:
 *     - modul_syneravip_akcija(string $akcija, array $podatki): array
 *
 * 👤 AVTOR:
...
function modul_syneravip_akcija(string $akcija, array $podatki = []): array {
    if (!Modul_Bridge::vloga_preveri('S0')) {
        return odziv_napaka('Dostop zavrnjen', 403);
    }

    return match($akcija) {
        'info'  => _modul_syneravip_akcija_info($podatki),
        'domov' => _modul_syneravip_akcija_domov($podatki),
...
function _modul_syneravip_akcija_info(array $podatki): array {
    return odziv_uspeh([
        'ime'       => 'SyneraVip',
        'id'        => 'syneravip',
        'verzija'   => '1.0.0',
        'opis'      => 'Modul SyneraVip',
        'uporabnik' => Modul_Bridge::uporabnik_pridobi()['ime'] ?? 'Gost',
    ], 'Informacije o modulu');
}

function _modul_syneravip_akcija_domov(array $podatki): array {
    return odziv_uspeh([
        'sporocilo' => 'Pozdravljen v modulu SyneraVip!',
        'cas'       => time(),
    ], 'Domov');
}
...
if (basename($_SERVER['SCRIPT_FILENAME'] ?? '') === 'modul.php' && !defined('SISTEM_OBSTAJA')) {
    ...
    $podatki = $_REQUEST;
    $odziv   = modul_syneravip_akcija($akcija, $podatki);
