 ...
 *
 * 📰 NAMEN:
 *     Drevo življenja — deset sefir, poti med njimi in kabalistična meditacija.
 *
 * 🔧 JAVNE FUNKCIJE:
 *     - modul_sephirot_akcija(string $akcija, array $podatki): array
 *
 * 📡 ODVISNOSTI:
 *     - Modul_Bridge
 *
 ...
 *
 * 🏷️ OZNAKE:
 *     modul, sephirot, hebrejskamistika
 * ============================================================
 */
...
function modul_sephirot_akcija(string $akcija, array $podatki = []): array {
    if (!Modul_Bridge::vloga_preveri('S2')) {
...
function _modul_sephirot_info(array $podatki): array {
    ...
    return odziv_uspeh([
        ...
        'kategorija'=> 'HebrejskaMistika',
        'verzija'   => '1.0.0',
        'opis'      => 'Drevo življenja — deset sefir, poti med njimi in kabalistična meditacija.',
        'uporabnik' => $uporabnik['ime'] ?? 'Gost',
    ], 'Informacije o modulu');
}

function _modul_sephirot_domov(array $podatki): array {
    return odziv_uspeh([
        'sporocilo' => 'Pozdravljen v modulu Sephirot!',
        'cas'       => time(),
    ], 'Domov');
}
...
if (basename($_SERVER['SCRIPT_FILENAME'] ?? '') === 'modul.php' && !defined('SISTEM_OBSTAJA')) {
    ...
    $odziv   = modul_sephirot_akcija($akcija, $_REQUEST);
