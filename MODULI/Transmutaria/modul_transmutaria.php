...
class ModulTransmutaria {
    ...
    public function __construct() {
        $this->db = new ModulTransmutariaJsonBaza();
        $this->config = $this->db->pridobiVse();
    }

    public function pridobiOsnovnePodatke() {
        return [
            'ime' => $this->config['ime'] ?? 'Transmutaria',
    ...
    public function pridobiVsebino($param = []) {
        $html = \"<div class='modul-transmutaria'>\" . htmlspecialchars($this->config['opis'] ?? '') . \"</div>\";
        return ['naslov' => $this->config['opis'] ?? 'Transmutaria', 'vsebina' => $html];
    }
