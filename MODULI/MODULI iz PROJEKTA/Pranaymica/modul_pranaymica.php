...
require_once __DIR__ . '/modul_pranaymica_funkcije.php';
require_once __DIR__ . '/modul_pranaymica_pravila.php';
require_once __DIR__ . '/modul_pranaymica_jsonbaza.php';

class ModulPranaymica {
    private $db;
    private $config;

    public function __construct() {
        $this->db = new ModulPranaymicaJsonBaza();
        $this->config = $this->db->pridobiVse();
    }

    public function pridobiOsnovnePodatke() {
        return [
            'ime' => $this->config['ime'] ?? 'Pranaymica',
    ...
    public function pridobiVsebino($param = []) {
        $html = \"<div class='modul-pranaymica'>\" . htmlspecialchars($this->config['opis'] ?? '') . \"</div>\";
        return ['naslov' => $this->config['opis'] ?? 'Pranaymica', 'vsebina' => $html];
    }
