...
class ModulOraculumVisionis {
    ...
    public function __construct() {
        $this->db = new ModulOraculumVisionisJsonBaza();
        $this->config = $this->db->pridobiVse();
    }

    public function pridobiOsnovnePodatke() {
        return [
            'ime' => $this->config['ime'] ?? 'OraculumVisionis',
    ...
    public function pridobiVsebino($param = []) {
        $html = \"<div class='modul-oraculumvisionis'>\" . htmlspecialchars($this->config['opis'] ?? '') . \"</div>\";
        return ['naslov' => $this->config['opis'] ?? 'OraculumVisionis', 'vsebina' => $html];
    }
