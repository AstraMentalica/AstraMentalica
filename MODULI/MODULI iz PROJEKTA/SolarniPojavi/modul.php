...
/**
 * MODUL: SolarniPojavi
 * POT: MODULI/SolarniPojavi/modul.php
 * VERZIJA: 1.0.0 (24.6.2026)
 *
 * 🏛️ NIVO: MODUL
 *
 * 📰 NAMEN:
 *     Standardna vstopna točka za SolarniPojavi modul
...
function modul_solarnipojavi_akcija(string $akcija, array $podatki = []): array {
    ...
    return match($akcija) {
        'info'  => _modul_solarnipojavi_akcija_info($podatki),
        'domov' => _modul_solarnipojavi_akcija_domov($podatki),
...
function _modul_solarnipojavi_akcija_info(array $podatki): array {
    return odziv_uspeh([
        'ime'       => 'SolarniPojavi',
        'id'        => 'solarnipojavi',
        'verzija'   => '1.0.0',
        'opis'      => 'Modul SolarniPojavi',
        'uporabnik' => Modul_Bridge::uporabnik_pridobi()['ime'] ?? 'Gost',
    ], 'Informacije o modulu');
}

function _modul_solarnipojavi_akcija_domov(array $podatki): array {
    return odziv_uspeh([
...
if (basename($_SERVER['SCRIPT_FILENAME'] ?? '') === 'modul.php' && !defined('SISTEM_OBSTAJA')) {
    ...
    $podatki = $_REQUEST;
    $odziv   = modul_solarnipojavi_akcija($akcija, $podatki);
