...
class ModulVibraMystica {
    ...
    public function __construct() {
        $this->db = new ModulVibraMysticaJsonBaza();
        $this->config = $this->db->pridobiVse();
    }

    public function pridobiOsnovnePodatke() {
        return [
            'ime' => $this->config['ime'] ?? 'VibraMystica',
    ...
    public function pridobiVsebino($param = []) {
        $html = \"<div class='modul-vibramystica'>\" . htmlspecialchars($this->config['opis'] ?? '') . \"</div>\";
        return ['naslov' => $this->config['opis'] ?? 'VibraMystica', 'vsebina' => $html];
    }
