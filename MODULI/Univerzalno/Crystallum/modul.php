 ...
 *
 * 📰 NAMEN:
 *     Kristali — lastnosti, energije in uporaba kristalnih kamnin.
 *
 * 🔧 JAVNE FUNKCIJE:
 *     - modul_crystallum_akcija(string $akcija, array $podatki): array
 *
 * 📡 ODVISNOSTI:
 *     - Modul_Bridge
 *
...
function _modul_crystallum_info(array $podatki): array {
    ...
    return odziv_uspeh([
        ...
        'kategorija'=> 'Univerzalno',
        'verzija'   => '1.0.0',
        'opis'      => 'Kristali — lastnosti, energije in uporaba kristalnih kamnin.',
        'uporabnik' => $uporabnik['ime'] ?? 'Gost',
    ], 'Informacije o modulu');
}

function _modul_crystallum_domov(array $podatki): array {
    return odziv_uspeh([
        'sporocilo' => 'Pozdravljen v modulu Crystallum!',
        'cas'       => time(),
    ], 'Domov');
}
...
if (basename($_SERVER['SCRIPT_FILENAME'] ?? '') === 'modul.php' && !defined('SISTEM_OBSTAJA')) {
    ...
    $odziv   = modul_crystallum_akcija($akcija, $_REQUEST);
