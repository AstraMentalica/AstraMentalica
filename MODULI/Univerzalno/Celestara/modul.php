 ...
 *
 * 📰 NAMEN:
 *     Nebesna modrost — zvezde, ozvezdja, planetarne ure in kozmični ritmi.
 *
 * 🔧 JAVNE FUNKCIJE:
 *     - modul_celestara_akcija(string $akcija, array $podatki): array
 *
 * 📡 ODVISNOSTI:
 *     - Modul_Bridge
 *
...
function _modul_celestara_info(array $podatki): array {
    ...
    return odziv_uspeh([
        ...
        'kategorija'=> 'Univerzalno',
        'verzija'   => '1.0.0',
        'opis'      => 'Nebesna modrost — zvezde, ozvezdja, planetarne ure in kozmični ritmi.',
        'uporabnik' => $uporabnik['ime'] ?? 'Gost',
    ], 'Informacije o modulu');
}

function _modul_celestara_domov(array $podatki): array {
    return odziv_uspeh([
        'sporocilo' => 'Pozdravljen v modulu Celestara!',
        'cas'       => time(),
    ], 'Domov');
}
...
if (basename($_SERVER['SCRIPT_FILENAME'] ?? '') === 'modul.php' && !defined('SISTEM_OBSTAJA')) {
    ...
    $odziv   = modul_celestara_akcija($akcija, $_REQUEST);
