 ...
 *
 * 📰 NAMEN:
 *     Večnost — arhivirane resnice, časovne dimenzije in kozmična zavest.
 *
 * 🔧 JAVNE FUNKCIJE:
...
function _modul_aeternum_info(array $podatki): array {
    ...
    return odziv_uspeh([
        ...
        'kategorija'=> 'Univerzalno',
        'verzija'   => '1.0.0',
        'opis'      => 'Večnost — arhivirane resnice, časovne dimenzije in kozmična zavest.',
        'uporabnik' => $uporabnik['ime'] ?? 'Gost',
    ], 'Informacije o modulu');
}

function _modul_aeternum_domov(array $podatki): array {
    return odziv_uspeh([
...
if (basename($_SERVER['SCRIPT_FILENAME'] ?? '') === 'modul.php' && !defined('SISTEM_OBSTAJA')) {
    ...
    $odziv   = modul_aeternum_akcija($akcija, $_REQUEST);
