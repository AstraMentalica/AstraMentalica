 ...
 *
 * 📰 NAMEN:
 *     Veles — slovansko božanstvo podzemlja, narave in modrosti.
 *
 * 🔧 JAVNE FUNKCIJE:
 *     - modul_veles_akcija(string $akcija, array $podatki): array
 *
 * 📡 ODVISNOSTI:
 *     - Modul_Bridge
 *
...
function _modul_veles_info(array $podatki): array {
    ...
    return odziv_uspeh([
        ...
        'kategorija'=> 'EvropaSlovani',
        'verzija'   => '1.0.0',
        'opis'      => 'Veles — slovansko božanstvo podzemlja, narave in modrosti.',
        'uporabnik' => $uporabnik['ime'] ?? 'Gost',
    ], 'Informacije o modulu');
}

function _modul_veles_domov(array $podatki): array {
    return odziv_uspeh([
        'sporocilo' => 'Pozdravljen v modulu Veles!',
        'cas'       => time(),
    ], 'Domov');
}
...
if (basename($_SERVER['SCRIPT_FILENAME'] ?? '') === 'modul.php' && !defined('SISTEM_OBSTAJA')) {
    ...
    $odziv   = modul_veles_akcija($akcija, $_REQUEST);
