 ...
 *
 * 📰 NAMEN:
 *     Perun — slovansko božanstvo groma, pravice in nebesnih sil.
 *
 * 🔧 JAVNE FUNKCIJE:
 *     - modul_perun_akcija(string $akcija, array $podatki): array
 *
 * 📡 ODVISNOSTI:
 *     - Modul_Bridge
 *
...
function _modul_perun_info(array $podatki): array {
    ...
    return odziv_uspeh([
        ...
        'kategorija'=> 'EvropaSlovani',
        'verzija'   => '1.0.0',
        'opis'      => 'Perun — slovansko božanstvo groma, pravice in nebesnih sil.',
        'uporabnik' => $uporabnik['ime'] ?? 'Gost',
    ], 'Informacije o modulu');
}

function _modul_perun_domov(array $podatki): array {
    return odziv_uspeh([
        'sporocilo' => 'Pozdravljen v modulu Perun!',
        'cas'       => time(),
    ], 'Domov');
}
...
if (basename($_SERVER['SCRIPT_FILENAME'] ?? '') === 'modul.php' && !defined('SISTEM_OBSTAJA')) {
    ...
    $odziv   = modul_perun_akcija($akcija, $_REQUEST);
