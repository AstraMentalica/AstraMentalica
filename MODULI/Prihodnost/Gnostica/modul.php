 ...
 *
 * 📰 NAMEN:
 *     Gnosticizem — gnostični miti, Demiurg in iskanje božanske iskre.
 *
 * 🔧 JAVNE FUNKCIJE:
 *     - modul_gnostica_akcija(string $akcija, array $podatki): array
 *
 * 📡 ODVISNOSTI:
 *     - Modul_Bridge
 *
...
function _modul_gnostica_info(array $podatki): array {
    ...
    return odziv_uspeh([
        ...
        'kategorija'=> 'Prihodnost',
        'verzija'   => '1.0.0',
        'opis'      => 'Gnosticizem — gnostični miti, Demiurg in iskanje božanske iskre.',
        'uporabnik' => $uporabnik['ime'] ?? 'Gost',
    ], 'Informacije o modulu');
}

function _modul_gnostica_domov(array $podatki): array {
    return odziv_uspeh([
        'sporocilo' => 'Pozdravljen v modulu Gnostica!',
        'cas'       => time(),
    ], 'Domov');
}
...
if (basename($_SERVER['SCRIPT_FILENAME'] ?? '') === 'modul.php' && !defined('SISTEM_OBSTAJA')) {
    ...
    $odziv   = modul_gnostica_akcija($akcija, $_REQUEST);
