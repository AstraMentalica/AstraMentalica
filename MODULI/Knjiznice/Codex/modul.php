 ...
 *
 * 📰 NAMEN:
 *     Codex — osrednja knjižnica duhovnih besedil in komentarjev. Jedro znanja sistema.
 *
 * 🔧 JAVNE FUNKCIJE:
 *     - modul_codex_akcija(string $akcija, array $podatki): array
 *
 * 📡 ODVISNOSTI:
...
function _modul_codex_info(array $podatki): array {
    $uporabnik = Modul_Bridge::uporabnik_pridobi();
    return odziv_uspeh([
        'ime'       => 'Codex',
        'id'        => 'codex',
        'verzija'   => '1.0.0',
        'opis'      => 'Codex — osrednja knjižnica duhovnih besedil in komentarjev. Jedro znanja sistema.',
        'uporabnik' => $uporabnik['ime'] ?? 'Gost',
    ], 'Informacije o modulu');
}

function _modul_codex_domov(array $podatki): array {
    return odziv_uspeh([
        'sporocilo' => 'Pozdravljen v modulu Codex!',
        'cas'       => time(),
    ], 'Domov');
}
...
if (basename($_SERVER['SCRIPT_FILENAME'] ?? '') === 'modul.php' && !defined('SISTEM_OBSTAJA')) {
    ...
    $podatki = $_REQUEST;
    $odziv   = modul_codex_akcija($akcija, $podatki);
