 ...
 *
 * 📰 NAMEN:
 *     Oraculum Visionis — knjiga videnj in prerokovanj. Vizionarska besedila, sanje in prerokbe.
 *
 * 🔧 JAVNE FUNKCIJE:
 *     - modul_oraculumvisionis_akcija(string $akcija, array $podatki): array
 *
 * 📡 ODVISNOSTI:
...
function _modul_oraculumvisionis_info(array $podatki): array {
    $uporabnik = Modul_Bridge::uporabnik_pridobi();
    return odziv_uspeh([
        'ime'       => 'OraculumVisionis',
        'id'        => 'oraculumvisionis',
        'verzija'   => '1.0.0',
        'opis'      => 'Oraculum Visionis — knjiga videnj in prerokovanj. Vizionarska besedila, sanje in prerokbe.',
        'uporabnik' => $uporabnik['ime'] ?? 'Gost',
    ], 'Informacije o modulu');
}

function _modul_oraculumvisionis_domov(array $podatki): array {
    return odziv_uspeh([
        'sporocilo' => 'Pozdravljen v modulu OraculumVisionis!',
        'cas'       => time(),
    ], 'Domov');
}
...
if (basename($_SERVER['SCRIPT_FILENAME'] ?? '') === 'modul.php' && !defined('SISTEM_OBSTAJA')) {
    ...
    $podatki = $_REQUEST;
    $odziv   = modul_oraculumvisionis_akcija($akcija, $podatki);
