...
class ModulUmbraeCodex {
    ...
    public function __construct() {
        $this->db = new ModulUmbraeCodexJsonBaza();
        $this->config = $this->db->pridobiVse();
    }

    public function pridobiOsnovnePodatke(): array {
        return [
            'ime'            => $this->config['ime'] ?? 'UmbraeCodex',
            'opis'           => $this->config['opis'] ?? '',
            'razlicica'      => $this->config['razlicica'] ?? '1.0.0',
            'avtor'          => $this->config['avtor'] ?? '',
            'minimalna_vloga'=> $this->config['minimalna_vloga'] ?? 'S1',
        ];
    }

    public function pridobiVsebino(array $param = []): array {
        $html = "<div class='modul-umbraecodex'>" . htmlspecialchars($this->config['opis'] ?? '') . "</div>";
        return ['naslov' => $this->config['opis'] ?? 'UmbraeCodex', 'vsebina' => $html];
    }
