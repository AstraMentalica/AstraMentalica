...
/**
 * MODUL: SqiMapper
 * POT: MODULI/SqiMapper/modul.php
 * VERZIJA: 1.0.0 (24.6.2026)
 *
 * 🏛️ NIVO: MODUL
 *
 * 📰 NAMEN:
 *     Standardna vstopna točka za SqiMapper modul
 *     Avtomatsko generirano
 *
 * 🔧 JAVNE FUNKCIJE:
 *     - modul_sqimapper_akcija(string $akcija, array $podatki): array
 *
 * 👤 AVTOR:
...
function modul_sqimapper_akcija(string $akcija, array $podatki = []): array {
    if (!Modul_Bridge::vloga_preveri('S0')) {
        return odziv_napaka('Dostop zavrnjen', 403);
    }

    return match($akcija) {
        'info'  => _modul_sqimapper_akcija_info($podatki),
        'domov' => _modul_sqimapper_akcija_domov($podatki),
...
function _modul_sqimapper_akcija_info(array $podatki): array {
    return odziv_uspeh([
        'ime'       => 'SqiMapper',
        'id'        => 'sqimapper',
        'verzija'   => '1.0.0',
        'opis'      => 'Modul SqiMapper',
        'uporabnik' => Modul_Bridge::uporabnik_pridobi()['ime'] ?? 'Gost',
    ], 'Informacije o modulu');
}

function _modul_sqimapper_akcija_domov(array $podatki): array {
    return odziv_uspeh([
        'sporocilo' => 'Pozdravljen v modulu SqiMapper!',
        'cas'       => time(),
    ], 'Domov');
}
...
if (basename($_SERVER['SCRIPT_FILENAME'] ?? '') === 'modul.php' && !defined('SISTEM_OBSTAJA')) {
    ...
    $podatki = $_REQUEST;
    $odziv   = modul_sqimapper_akcija($akcija, $podatki);
