 ...
 *
 * 📰 NAMEN:
 *     Vedska astrologija — jyotish, nakshatre in vedske planetarne energije.
 *
 * 🔧 JAVNE FUNKCIJE:
 *     - modul_jyotisha_akcija(string $akcija, array $podatki): array
 *
 * 📡 ODVISNOSTI:
 *     - Modul_Bridge
 *
...
function _modul_jyotisha_info(array $podatki): array {
    ...
    return odziv_uspeh([
        ...
        'kategorija'=> 'Indija',
        'verzija'   => '1.0.0',
        'opis'      => 'Vedska astrologija — jyotish, nakshatre in vedske planetarne energije.',
        'uporabnik' => $uporabnik['ime'] ?? 'Gost',
    ], 'Informacije o modulu');
}

function _modul_jyotisha_domov(array $podatki): array {
    return odziv_uspeh([
        'sporocilo' => 'Pozdravljen v modulu Jyotisha!',
        'cas'       => time(),
    ], 'Domov');
}
...
if (basename($_SERVER['SCRIPT_FILENAME'] ?? '') === 'modul.php' && !defined('SISTEM_OBSTAJA')) {
    ...
    $odziv   = modul_jyotisha_akcija($akcija, $_REQUEST);
