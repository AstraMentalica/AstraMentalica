...
/**
 * MODUL: ViaAnimae
 * POT: MODULI/Samanizem/ViaAnimae/modul.php
 * 📅 VERZIJA: v1.0.0 (2026-06-28)
 * 📰 NAMEN: Pot duše — šamanistična tradicija poti duše in medduhovnih potovanj.
 * 👤 AVTOR: Damir Šafarič
 */
...
function modul_viaanimae_akcija(string $akcija, array $podatki = []): array {
    if (!Modul_Bridge::vloga_preveri("S0")) return odziv_napaka("Dostop zavrnjen", 403);
    return match($akcija) {
        "info"  => _modul_viaanimae_info($podatki),
        "domov" => _modul_viaanimae_domov($podatki),
        default => odziv_napaka("Neznana akcija: $akcija", 400),
    };
}

function _modul_viaanimae_info(array $p): array {
    return odziv_uspeh(["ime"=>"ViaAnimae","id"=>"viaanimae","verzija"=>"1.0.0","opis"=>"Pot duše — šamanistična tradicija poti duše in medduhovnih potovanj.","uporabnik"=>Modul_Bridge::uporabnik_pridobi()["ime"] ?? "Gost"], "Informacije");
}
function _modul_viaanimae_domov(array $p): array {
    return odziv_uspeh(["sporocilo"=>"Pozdravljen v ViaAnimae!","cas"=>time()], "Domov");
}

if (basename($_SERVER["SCRIPT_FILENAME"] ?? "") === "modul.php" && !defined("SISTEM_OBSTAJA")) {
    $odziv = modul_viaanimae_akcija($_REQUEST["akcija"] ?? "domov", $_REQUEST);
