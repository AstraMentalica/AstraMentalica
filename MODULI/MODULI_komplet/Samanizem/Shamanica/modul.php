...
/**
 * MODUL: Shamanica
 * POT: MODULI/Samanizem/Shamanica/modul.php
 * 📅 VERZIJA: v1.0.0 (2026-06-28)
 * 📰 NAMEN: Šamanizem — šamanski rituali, bobni, duhovna potovanja in zdravljenje.
 * 👤 AVTOR: Damir Šafarič
 */
...
function modul_shamanica_akcija(string $akcija, array $podatki = []): array {
    ...
    return match($akcija) {
        "info"  => _modul_shamanica_info($podatki),
        "domov" => _modul_shamanica_domov($podatki),
        default => odziv_napaka("Neznana akcija: $akcija", 400),
    };
}

function _modul_shamanica_info(array $p): array {
    return odziv_uspeh(["ime"=>"Shamanica","id"=>"shamanica","verzija"=>"1.0.0","opis"=>"Šamanizem — šamanski rituali, bobni, duhovna potovanja in zdravljenje.","uporabnik"=>Modul_Bridge::uporabnik_pridobi()["ime"] ?? "Gost"], "Informacije");
}
function _modul_shamanica_domov(array $p): array {
    return odziv_uspeh(["sporocilo"=>"Pozdravljen v Shamanica!","cas"=>time()], "Domov");
}

if (basename($_SERVER["SCRIPT_FILENAME"] ?? "") === "modul.php" && !defined("SISTEM_OBSTAJA")) {
    $odziv = modul_shamanica_akcija($_REQUEST["akcija"] ?? "domov", $_REQUEST);
