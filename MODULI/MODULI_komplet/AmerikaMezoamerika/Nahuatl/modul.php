...
/**
 * MODUL: Nahuatl
 * POT: MODULI/AmerikaMezoamerika/Nahuatl/modul.php
 * 📅 VERZIJA: v1.0.0 (2026-06-28)
 * 📰 NAMEN: Nahuatl — jezik in modrost nahuatl tradicije, tonal in nagual.
 * 👤 AVTOR: Damir Šafarič
 */
...
function modul_nahuatl_akcija(string $akcija, array $podatki = []): array {
    if (!Modul_Bridge::vloga_preveri("S0")) return odziv_napaka("Dostop zavrnjen", 403);
    return match($akcija) {
        "info"  => _modul_nahuatl_info($podatki),
        "domov" => _modul_nahuatl_domov($podatki),
        default => odziv_napaka("Neznana akcija: $akcija", 400),
    };
}

function _modul_nahuatl_info(array $p): array {
    return odziv_uspeh(["ime"=>"Nahuatl","id"=>"nahuatl","verzija"=>"1.0.0","opis"=>"Nahuatl — jezik in modrost nahuatl tradicije, tonal in nagual.","uporabnik"=>Modul_Bridge::uporabnik_pridobi()["ime"] ?? "Gost"], "Informacije");
}
function _modul_nahuatl_domov(array $p): array {
    return odziv_uspeh(["sporocilo"=>"Pozdravljen v Nahuatl!","cas"=>time()], "Domov");
}

if (basename($_SERVER["SCRIPT_FILENAME"] ?? "") === "modul.php" && !defined("SISTEM_OBSTAJA")) {
    $odziv = modul_nahuatl_akcija($_REQUEST["akcija"] ?? "domov", $_REQUEST);
