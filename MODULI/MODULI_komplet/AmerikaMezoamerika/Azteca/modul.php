 ...
 * 📰 NAMEN: Azteka — aztekski panteon, kozmogonija in duhovna tradicija.
 * 👤 AVTOR: Damir Šafarič
 */
...
function _modul_azteca_info(array $p): array {
    return odziv_uspeh(["ime"=>"Azteca","id"=>"azteca","verzija"=>"1.0.0","opis"=>"Azteka — aztekski panteon, kozmogonija in duhovna tradicija.","uporabnik"=>Modul_Bridge::uporabnik_pridobi()["ime"] ?? "Gost"], "Informacije");
}
function _modul_azteca_domov(array $p): array {
    return odziv_uspeh(["sporocilo"=>"Pozdravljen v Azteca!","cas"=>time()], "Domov");
}

if (basename($_SERVER["SCRIPT_FILENAME"] ?? "") === "modul.php" && !defined("SISTEM_OBSTAJA")) {
    $odziv = modul_azteca_akcija($_REQUEST["akcija"] ?? "domov", $_REQUEST);
