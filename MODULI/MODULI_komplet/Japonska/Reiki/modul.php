...
/**
 * MODUL: Reiki
 * POT: MODULI/Japonska/Reiki/modul.php
 * 📅 VERZIJA: v1.0.0 (2026-06-28)
 * 📰 NAMEN: Reiki — japonska tehnika energijskega zdravljenja in ravnovesja.
 * 👤 AVTOR: Damir Šafarič
 */
...
function modul_reiki_akcija(string $akcija, array $podatki = []): array {
    if (!Modul_Bridge::vloga_preveri("S0")) return odziv_napaka("Dostop zavrnjen", 403);
    return match($akcija) {
        "info"  => _modul_reiki_info($podatki),
        "domov" => _modul_reiki_domov($podatki),
        default => odziv_napaka("Neznana akcija: $akcija", 400),
    };
}

function _modul_reiki_info(array $p): array {
    return odziv_uspeh(["ime"=>"Reiki","id"=>"reiki","verzija"=>"1.0.0","opis"=>"Reiki — japonska tehnika energijskega zdravljenja in ravnovesja.","uporabnik"=>Modul_Bridge::uporabnik_pridobi()["ime"] ?? "Gost"], "Informacije");
}
function _modul_reiki_domov(array $p): array {
    return odziv_uspeh(["sporocilo"=>"Pozdravljen v Reiki!","cas"=>time()], "Domov");
}

if (basename($_SERVER["SCRIPT_FILENAME"] ?? "") === "modul.php" && !defined("SISTEM_OBSTAJA")) {
    $odziv = modul_reiki_akcija($_REQUEST["akcija"] ?? "domov", $_REQUEST);
