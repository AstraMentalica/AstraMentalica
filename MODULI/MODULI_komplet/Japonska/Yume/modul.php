...
/**
 * MODUL: Yume
 * POT: MODULI/Japonska/Yume/modul.php
 * 📅 VERZIJA: v1.0.0 (2026-06-28)
 * 📰 NAMEN: Svet sanj — japonska tradicija sanjske interpretacije in yume no michi.
 * 👤 AVTOR: Damir Šafarič
 */
...
function modul_yume_akcija(string $akcija, array $podatki = []): array {
    if (!Modul_Bridge::vloga_preveri("S0")) return odziv_napaka("Dostop zavrnjen", 403);
    return match($akcija) {
        "info"  => _modul_yume_info($podatki),
        "domov" => _modul_yume_domov($podatki),
        default => odziv_napaka("Neznana akcija: $akcija", 400),
    };
}

function _modul_yume_info(array $p): array {
    return odziv_uspeh(["ime"=>"Yume","id"=>"yume","verzija"=>"1.0.0","opis"=>"Svet sanj — japonska tradicija sanjske interpretacije in yume no michi.","uporabnik"=>Modul_Bridge::uporabnik_pridobi()["ime"] ?? "Gost"], "Informacije");
}
function _modul_yume_domov(array $p): array {
    return odziv_uspeh(["sporocilo"=>"Pozdravljen v Yume!","cas"=>time()], "Domov");
}

if (basename($_SERVER["SCRIPT_FILENAME"] ?? "") === "modul.php" && !defined("SISTEM_OBSTAJA")) {
    $odziv = modul_yume_akcija($_REQUEST["akcija"] ?? "domov", $_REQUEST);
