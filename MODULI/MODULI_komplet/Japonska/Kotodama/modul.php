...
/**
 * MODUL: Kotodama
 * POT: MODULI/Japonska/Kotodama/modul.php
 * 📅 VERZIJA: v1.0.0 (2026-06-28)
 * 📰 NAMEN: Moč besed — japonska tradicija zvočne magije in moči besed.
 * 👤 AVTOR: Damir Šafarič
 */
...
function modul_kotodama_akcija(string $akcija, array $podatki = []): array {
    if (!Modul_Bridge::vloga_preveri("S0")) return odziv_napaka("Dostop zavrnjen", 403);
    return match($akcija) {
        "info"  => _modul_kotodama_info($podatki),
        "domov" => _modul_kotodama_domov($podatki),
        default => odziv_napaka("Neznana akcija: $akcija", 400),
    };
}

function _modul_kotodama_info(array $p): array {
    return odziv_uspeh(["ime"=>"Kotodama","id"=>"kotodama","verzija"=>"1.0.0","opis"=>"Moč besed — japonska tradicija zvočne magije in moči besed.","uporabnik"=>Modul_Bridge::uporabnik_pridobi()["ime"] ?? "Gost"], "Informacije");
}
function _modul_kotodama_domov(array $p): array {
    return odziv_uspeh(["sporocilo"=>"Pozdravljen v Kotodama!","cas"=>time()], "Domov");
}

if (basename($_SERVER["SCRIPT_FILENAME"] ?? "") === "modul.php" && !defined("SISTEM_OBSTAJA")) {
    $odziv = modul_kotodama_akcija($_REQUEST["akcija"] ?? "domov", $_REQUEST);
