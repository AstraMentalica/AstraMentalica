...
/**
 * MODUL: Runir
 * POT: MODULI/EvropaNordija/Runir/modul.php
 * 📅 VERZIJA: v1.0.0 (2026-06-28)
 * 📰 NAMEN: Rune — nordijska runska pisava, Elder Futhark in runska magija.
 * 👤 AVTOR: Damir Šafarič
 */
...
function modul_runir_akcija(string $akcija, array $podatki = []): array {
    if (!Modul_Bridge::vloga_preveri("S0")) return odziv_napaka("Dostop zavrnjen", 403);
    return match($akcija) {
        "info"  => _modul_runir_info($podatki),
        "domov" => _modul_runir_domov($podatki),
        default => odziv_napaka("Neznana akcija: $akcija", 400),
    };
}

function _modul_runir_info(array $p): array {
    return odziv_uspeh(["ime"=>"Runir","id"=>"runir","verzija"=>"1.0.0","opis"=>"Rune — nordijska runska pisava, Elder Futhark in runska magija.","uporabnik"=>Modul_Bridge::uporabnik_pridobi()["ime"] ?? "Gost"], "Informacije");
}
function _modul_runir_domov(array $p): array {
    return odziv_uspeh(["sporocilo"=>"Pozdravljen v Runir!","cas"=>time()], "Domov");
}

if (basename($_SERVER["SCRIPT_FILENAME"] ?? "") === "modul.php" && !defined("SISTEM_OBSTAJA")) {
    $odziv = modul_runir_akcija($_REQUEST["akcija"] ?? "domov", $_REQUEST);
