...
/**
 * MODUL: Musok
 * POT: MODULI/Koreja/Musok/modul.php
 * 📅 VERZIJA: v1.0.0 (2026-06-28)
 * 📰 NAMEN: Korejski šamanizem — mudang, duhovi prednikov in korejske duhovne prakse.
 * 👤 AVTOR: Damir Šafarič
 */
...
function modul_musok_akcija(string $akcija, array $podatki = []): array {
    if (!Modul_Bridge::vloga_preveri("S0")) return odziv_napaka("Dostop zavrnjen", 403);
    return match($akcija) {
        "info"  => _modul_musok_info($podatki),
        "domov" => _modul_musok_domov($podatki),
        default => odziv_napaka("Neznana akcija: $akcija", 400),
    };
}

function _modul_musok_info(array $p): array {
    return odziv_uspeh(["ime"=>"Musok","id"=>"musok","verzija"=>"1.0.0","opis"=>"Korejski šamanizem — mudang, duhovi prednikov in korejske duhovne prakse.","uporabnik"=>Modul_Bridge::uporabnik_pridobi()["ime"] ?? "Gost"], "Informacije");
}
function _modul_musok_domov(array $p): array {
    return odziv_uspeh(["sporocilo"=>"Pozdravljen v Musok!","cas"=>time()], "Domov");
}

if (basename($_SERVER["SCRIPT_FILENAME"] ?? "") === "modul.php" && !defined("SISTEM_OBSTAJA")) {
    $odziv = modul_musok_akcija($_REQUEST["akcija"] ?? "domov", $_REQUEST);
