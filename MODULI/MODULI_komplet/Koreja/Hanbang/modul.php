...
/**
 * MODUL: Hanbang
 * POT: MODULI/Koreja/Hanbang/modul.php
 * 📅 VERZIJA: v1.0.0 (2026-06-28)
 * 📰 NAMEN: Tradicionalna korejska medicina — hanbang, zeliščarstvo in energijsko zdravljenje.
 * 👤 AVTOR: Damir Šafarič
 */
...
function modul_hanbang_akcija(string $akcija, array $podatki = []): array {
    if (!Modul_Bridge::vloga_preveri("S0")) return odziv_napaka("Dostop zavrnjen", 403);
    return match($akcija) {
        "info"  => _modul_hanbang_info($podatki),
        "domov" => _modul_hanbang_domov($podatki),
        default => odziv_napaka("Neznana akcija: $akcija", 400),
    };
}

function _modul_hanbang_info(array $p): array {
    return odziv_uspeh(["ime"=>"Hanbang","id"=>"hanbang","verzija"=>"1.0.0","opis"=>"Tradicionalna korejska medicina — hanbang, zeliščarstvo in energijsko zdravljenje.","uporabnik"=>Modul_Bridge::uporabnik_pridobi()["ime"] ?? "Gost"], "Informacije");
}
function _modul_hanbang_domov(array $p): array {
    return odziv_uspeh(["sporocilo"=>"Pozdravljen v Hanbang!","cas"=>time()], "Domov");
}

if (basename($_SERVER["SCRIPT_FILENAME"] ?? "") === "modul.php" && !defined("SISTEM_OBSTAJA")) {
    $odziv = modul_hanbang_akcija($_REQUEST["akcija"] ?? "domov", $_REQUEST);
