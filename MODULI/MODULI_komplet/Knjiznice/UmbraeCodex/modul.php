 ...
 * 📰 NAMEN: Kodeks senc — senca po Jungu, integracija temne strani.
 * 👤 AVTOR: Damir Šafarič
 */
...
function _modul_umbraecodex_info(array $p): array {
    return odziv_uspeh(["ime"=>"UmbraeCodex","id"=>"umbraecodex","verzija"=>"1.0.0","opis"=>"Kodeks senc — senca po Jungu, integracija temne strani.","uporabnik"=>Modul_Bridge::uporabnik_pridobi()["ime"] ?? "Gost"], "Informacije");
}
function _modul_umbraecodex_domov(array $p): array {
    return odziv_uspeh(["sporocilo"=>"Pozdravljen v UmbraeCodex!","cas"=>time()], "Domov");
}

if (basename($_SERVER["SCRIPT_FILENAME"] ?? "") === "modul.php" && !defined("SISTEM_OBSTAJA")) {
    $odziv = modul_umbraecodex_akcija($_REQUEST["akcija"] ?? "domov", $_REQUEST);
