 ...
 * 📰 NAMEN: Starodavni kodeksi — zbirka starih tekstov, prepisov in komentarjev.
 * 👤 AVTOR: Damir Šafarič
 */
...
function _modul_codexantiqua_info(array $p): array {
    return odziv_uspeh(["ime"=>"CodexAntiqua","id"=>"codexantiqua","verzija"=>"1.0.0","opis"=>"Starodavni kodeksi — zbirka starih tekstov, prepisov in komentarjev.","uporabnik"=>Modul_Bridge::uporabnik_pridobi()["ime"] ?? "Gost"], "Informacije");
}
function _modul_codexantiqua_domov(array $p): array {
    return odziv_uspeh(["sporocilo"=>"Pozdravljen v CodexAntiqua!","cas"=>time()], "Domov");
}

if (basename($_SERVER["SCRIPT_FILENAME"] ?? "") === "modul.php" && !defined("SISTEM_OBSTAJA")) {
    $odziv = modul_codexantiqua_akcija($_REQUEST["akcija"] ?? "domov", $_REQUEST);
