...
/**
 * MODUL: Codex
 * POT: MODULI/Knjiznice/Codex/modul.php
 * 📅 VERZIJA: v1.0.0 (2026-06-28)
 * 📰 NAMEN: Univerzalni kodeks — enciklopedija vseh tradicij, pojmov in simbolov.
 * 👤 AVTOR: Damir Šafarič
 */
...
function modul_codex_akcija(string $akcija, array $podatki = []): array {
    ...
    return match($akcija) {
        "info"  => _modul_codex_info($podatki),
        "domov" => _modul_codex_domov($podatki),
        default => odziv_napaka("Neznana akcija: $akcija", 400),
    };
}

function _modul_codex_info(array $p): array {
    return odziv_uspeh(["ime"=>"Codex","id"=>"codex","verzija"=>"1.0.0","opis"=>"Univerzalni kodeks — enciklopedija vseh tradicij, pojmov in simbolov.","uporabnik"=>Modul_Bridge::uporabnik_pridobi()["ime"] ?? "Gost"], "Informacije");
}
function _modul_codex_domov(array $p): array {
    return odziv_uspeh(["sporocilo"=>"Pozdravljen v Codex!","cas"=>time()], "Domov");
}

if (basename($_SERVER["SCRIPT_FILENAME"] ?? "") === "modul.php" && !defined("SISTEM_OBSTAJA")) {
    $odziv = modul_codex_akcija($_REQUEST["akcija"] ?? "domov", $_REQUEST);
