...
/**
 * MODUL: Orakleum
 * POT: MODULI/Knjiznice/Orakleum/modul.php
 * 📅 VERZIJA: v1.0.0 (2026-06-28)
 * 📰 NAMEN: Oraklji in prerokbe — zbirka orakljev različnih tradicij.
 * 👤 AVTOR: Damir Šafarič
 */
...
function modul_orakleum_akcija(string $akcija, array $podatki = []): array {
    ...
    return match($akcija) {
        "info"  => _modul_orakleum_info($podatki),
        "domov" => _modul_orakleum_domov($podatki),
        default => odziv_napaka("Neznana akcija: $akcija", 400),
    };
}

function _modul_orakleum_info(array $p): array {
    return odziv_uspeh(["ime"=>"Orakleum","id"=>"orakleum","verzija"=>"1.0.0","opis"=>"Oraklji in prerokbe — zbirka orakljev različnih tradicij.","uporabnik"=>Modul_Bridge::uporabnik_pridobi()["ime"] ?? "Gost"], "Informacije");
}
function _modul_orakleum_domov(array $p): array {
    return odziv_uspeh(["sporocilo"=>"Pozdravljen v Orakleum!","cas"=>time()], "Domov");
}

if (basename($_SERVER["SCRIPT_FILENAME"] ?? "") === "modul.php" && !defined("SISTEM_OBSTAJA")) {
    $odziv = modul_orakleum_akcija($_REQUEST["akcija"] ?? "domov", $_REQUEST);
