...
/**
 * MODUL: Stelaris
 * POT: MODULI/Znanost/Stelaris/modul.php
 * 📅 VERZIJA: v1.0.0 (2026-06-28)
 * 📰 NAMEN: Zvezdno nebo — astrološki motor, natalni horoskop, tranziti in aspekti.
 * 👤 AVTOR: Damir Šafarič
 */
...
function _modul_stelaris_info(array $p): array {
    return odziv_uspeh(["ime"=>"Stelaris","id"=>"stelaris","verzija"=>"1.0.0","opis"=>"Zvezdno nebo — astrološki motor, natalni horoskop, tranziti in aspekti.","uporabnik"=>Modul_Bridge::uporabnik_pridobi()["ime"] ?? "Gost"], "Informacije");
}
function _modul_stelaris_domov(array $p): array {
    return odziv_uspeh(["sporocilo"=>"Pozdravljen v Stelaris!","cas"=>time()], "Domov");
}

if (basename($_SERVER["SCRIPT_FILENAME"] ?? "") === "modul.php" && !defined("SISTEM_OBSTAJA")) {
    $odziv = modul_stelaris_akcija($_REQUEST["akcija"] ?? "domov", $_REQUEST);
