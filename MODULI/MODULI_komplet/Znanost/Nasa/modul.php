...
/**
 * MODUL: Nasa
 * POT: MODULI/Znanost/Nasa/modul.php
 * 📅 VERZIJA: v1.0.0 (2026-06-28)
 * 📰 NAMEN: NASA senzorji — sončeva aktivnost (NASA DONKI), geomagnetna aktivnost (NOAA SWPC).
 * 👤 AVTOR: Damir Šafarič
 */
...
function modul_nasa_akcija(string $akcija, array $podatki = []): array {
    if (!Modul_Bridge::vloga_preveri("S0")) return odziv_napaka("Dostop zavrnjen", 403);
    return match($akcija) {
        "info"  => _modul_nasa_info($podatki),
        "domov" => _modul_nasa_domov($podatki),
        default => odziv_napaka("Neznana akcija: $akcija", 400),
    };
}

function _modul_nasa_info(array $p): array {
    return odziv_uspeh(["ime"=>"Nasa","id"=>"nasa","verzija"=>"1.0.0","opis"=>"NASA senzorji — sončeva aktivnost (NASA DONKI), geomagnetna aktivnost (NOAA SWPC).","uporabnik"=>Modul_Bridge::uporabnik_pridobi()["ime"] ?? "Gost"], "Informacije");
}
function _modul_nasa_domov(array $p): array {
    return odziv_uspeh(["sporocilo"=>"Pozdravljen v Nasa!","cas"=>time()], "Domov");
}

if (basename($_SERVER["SCRIPT_FILENAME"] ?? "") === "modul.php" && !defined("SISTEM_OBSTAJA")) {
    $odziv = modul_nasa_akcija($_REQUEST["akcija"] ?? "domov", $_REQUEST);
