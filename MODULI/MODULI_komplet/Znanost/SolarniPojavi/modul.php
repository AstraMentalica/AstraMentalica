 ...
 * 📰 NAMEN: Sončevi pojavi — analiza sončevih izbruhov, CME in njihov vpliv.
 * 👤 AVTOR: Damir Šafarič
 */
...
function _modul_solarnipojavi_info(array $p): array {
    return odziv_uspeh(["ime"=>"SolarniPojavi","id"=>"solarnipojavi","verzija"=>"1.0.0","opis"=>"Sončevi pojavi — analiza sončevih izbruhov, CME in njihov vpliv.","uporabnik"=>Modul_Bridge::uporabnik_pridobi()["ime"] ?? "Gost"], "Informacije");
}
function _modul_solarnipojavi_domov(array $p): array {
    return odziv_uspeh(["sporocilo"=>"Pozdravljen v SolarniPojavi!","cas"=>time()], "Domov");
}

if (basename($_SERVER["SCRIPT_FILENAME"] ?? "") === "modul.php" && !defined("SISTEM_OBSTAJA")) {
    $odziv = modul_solarnipojavi_akcija($_REQUEST["akcija"] ?? "domov", $_REQUEST);
