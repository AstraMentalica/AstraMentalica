 ...
 * 📰 NAMEN: Sveta geometrija — kozmični vzorci, mandala, Flor življenja in fraktali.
 * 👤 AVTOR: Damir Šafarič
 */
...
function _modul_geometriasacra_info(array $p): array {
    return odziv_uspeh(["ime"=>"GeometriaSacra","id"=>"geometriasacra","verzija"=>"1.0.0","opis"=>"Sveta geometrija — kozmični vzorci, mandala, Flor življenja in fraktali.","uporabnik"=>Modul_Bridge::uporabnik_pridobi()["ime"] ?? "Gost"], "Informacije");
}
function _modul_geometriasacra_domov(array $p): array {
    return odziv_uspeh(["sporocilo"=>"Pozdravljen v GeometriaSacra!","cas"=>time()], "Domov");
}

if (basename($_SERVER["SCRIPT_FILENAME"] ?? "") === "modul.php" && !defined("SISTEM_OBSTAJA")) {
    $odziv = modul_geometriasacra_akcija($_REQUEST["akcija"] ?? "domov", $_REQUEST);
