 ...
 * 📰 NAMEN: Lunarne energije — lunine faze, rituali in ženski cikli.
 * 👤 AVTOR: Damir Šafarič
 */
...
function _modul_lunaris_info(array $p): array {
    return odziv_uspeh(["ime"=>"Lunaris","id"=>"lunaris","verzija"=>"1.0.0","opis"=>"Lunarne energije — lunine faze, rituali in ženski cikli.","uporabnik"=>Modul_Bridge::uporabnik_pridobi()["ime"] ?? "Gost"], "Informacije");
}
function _modul_lunaris_domov(array $p): array {
    return odziv_uspeh(["sporocilo"=>"Pozdravljen v Lunaris!","cas"=>time()], "Domov");
}

if (basename($_SERVER["SCRIPT_FILENAME"] ?? "") === "modul.php" && !defined("SISTEM_OBSTAJA")) {
    $odziv = modul_lunaris_akcija($_REQUEST["akcija"] ?? "domov", $_REQUEST);
