...
/**
 * MODUL: Aeternum
 * POT: MODULI/Univerzalno/Aeternum/modul.php
 * 📅 VERZIJA: v1.0.0 (2026-06-28)
 * 📰 NAMEN: Večnost — arhivirane resnice, časovne dimenzije in kozmična zavest.
 * 👤 AVTOR: Damir Šafarič
 */
...
function modul_aeternum_akcija(string $akcija, array $podatki = []): array {
    ...
    return match($akcija) {
        "info"  => _modul_aeternum_info($podatki),
        "domov" => _modul_aeternum_domov($podatki),
        default => odziv_napaka("Neznana akcija: $akcija", 400),
    };
}

function _modul_aeternum_info(array $p): array {
    return odziv_uspeh(["ime"=>"Aeternum","id"=>"aeternum","verzija"=>"1.0.0","opis"=>"Večnost — arhivirane resnice, časovne dimenzije in kozmična zavest.","uporabnik"=>Modul_Bridge::uporabnik_pridobi()["ime"] ?? "Gost"], "Informacije");
}
function _modul_aeternum_domov(array $p): array {
    return odziv_uspeh(["sporocilo"=>"Pozdravljen v Aeternum!","cas"=>time()], "Domov");
}

if (basename($_SERVER["SCRIPT_FILENAME"] ?? "") === "modul.php" && !defined("SISTEM_OBSTAJA")) {
    $odziv = modul_aeternum_akcija($_REQUEST["akcija"] ?? "domov", $_REQUEST);
