...
/**
 * MODUL: Celestara
 * POT: MODULI/Univerzalno/Celestara/modul.php
 * 📅 VERZIJA: v1.0.0 (2026-06-28)
 * 📰 NAMEN: Nebesna modrost — zvezde, ozvezdja, planetarne ure in kozmični ritmi.
 * 👤 AVTOR: Damir Šafarič
 */
...
function modul_celestara_akcija(string $akcija, array $podatki = []): array {
    if (!Modul_Bridge::vloga_preveri("S0")) return odziv_napaka("Dostop zavrnjen", 403);
    return match($akcija) {
        "info"  => _modul_celestara_info($podatki),
        "domov" => _modul_celestara_domov($podatki),
        default => odziv_napaka("Neznana akcija: $akcija", 400),
    };
}

function _modul_celestara_info(array $p): array {
    return odziv_uspeh(["ime"=>"Celestara","id"=>"celestara","verzija"=>"1.0.0","opis"=>"Nebesna modrost — zvezde, ozvezdja, planetarne ure in kozmični ritmi.","uporabnik"=>Modul_Bridge::uporabnik_pridobi()["ime"] ?? "Gost"], "Informacije");
}
function _modul_celestara_domov(array $p): array {
    return odziv_uspeh(["sporocilo"=>"Pozdravljen v Celestara!","cas"=>time()], "Domov");
}

if (basename($_SERVER["SCRIPT_FILENAME"] ?? "") === "modul.php" && !defined("SISTEM_OBSTAJA")) {
    $odziv = modul_celestara_akcija($_REQUEST["akcija"] ?? "domov", $_REQUEST);
