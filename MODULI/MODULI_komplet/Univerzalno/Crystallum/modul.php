...
/**
 * MODUL: Crystallum
 * POT: MODULI/Univerzalno/Crystallum/modul.php
 * 📅 VERZIJA: v1.0.0 (2026-06-28)
 * 📰 NAMEN: Kristali — lastnosti, energije in uporaba kristalnih kamnin.
 * 👤 AVTOR: Damir Šafarič
 */
...
function modul_crystallum_akcija(string $akcija, array $podatki = []): array {
    if (!Modul_Bridge::vloga_preveri("S0")) return odziv_napaka("Dostop zavrnjen", 403);
    return match($akcija) {
        "info"  => _modul_crystallum_info($podatki),
        "domov" => _modul_crystallum_domov($podatki),
        default => odziv_napaka("Neznana akcija: $akcija", 400),
    };
}

function _modul_crystallum_info(array $p): array {
    return odziv_uspeh(["ime"=>"Crystallum","id"=>"crystallum","verzija"=>"1.0.0","opis"=>"Kristali — lastnosti, energije in uporaba kristalnih kamnin.","uporabnik"=>Modul_Bridge::uporabnik_pridobi()["ime"] ?? "Gost"], "Informacije");
}
function _modul_crystallum_domov(array $p): array {
    return odziv_uspeh(["sporocilo"=>"Pozdravljen v Crystallum!","cas"=>time()], "Domov");
}

if (basename($_SERVER["SCRIPT_FILENAME"] ?? "") === "modul.php" && !defined("SISTEM_OBSTAJA")) {
    $odziv = modul_crystallum_akcija($_REQUEST["akcija"] ?? "domov", $_REQUEST);
