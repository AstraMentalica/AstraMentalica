...
/**
 * MODUL: Occultum
 * POT: MODULI/Univerzalno/Occultum/modul.php
 * 📅 VERZIJA: v1.0.0 (2026-06-28)
 * 📰 NAMEN: Okultne vede — skrito znanje, magijski sistemi in ezoterika.
 * 👤 AVTOR: Damir Šafarič
 */
...
function modul_occultum_akcija(string $akcija, array $podatki = []): array {
    if (!Modul_Bridge::vloga_preveri("S2")) return odziv_napaka("Dostop zavrnjen", 403);
    return match($akcija) {
        "info"  => _modul_occultum_info($podatki),
        "domov" => _modul_occultum_domov($podatki),
        default => odziv_napaka("Neznana akcija: $akcija", 400),
    };
}

function _modul_occultum_info(array $p): array {
    return odziv_uspeh(["ime"=>"Occultum","id"=>"occultum","verzija"=>"1.0.0","opis"=>"Okultne vede — skrito znanje, magijski sistemi in ezoterika.","uporabnik"=>Modul_Bridge::uporabnik_pridobi()["ime"] ?? "Gost"], "Informacije");
}
function _modul_occultum_domov(array $p): array {
    return odziv_uspeh(["sporocilo"=>"Pozdravljen v Occultum!","cas"=>time()], "Domov");
}

if (basename($_SERVER["SCRIPT_FILENAME"] ?? "") === "modul.php" && !defined("SISTEM_OBSTAJA")) {
    $odziv = modul_occultum_akcija($_REQUEST["akcija"] ?? "domov", $_REQUEST);
