...
/**
 * MODUL: Perun
 * POT: MODULI/EvropaSlovani/Perun/modul.php
 * 📅 VERZIJA: v1.0.0 (2026-06-28)
 * 📰 NAMEN: Perun — slovansko božanstvo groma, pravice in nebesnih sil.
 * 👤 AVTOR: Damir Šafarič
 */
...
function modul_perun_akcija(string $akcija, array $podatki = []): array {
    if (!Modul_Bridge::vloga_preveri("S0")) return odziv_napaka("Dostop zavrnjen", 403);
    return match($akcija) {
        "info"  => _modul_perun_info($podatki),
        "domov" => _modul_perun_domov($podatki),
        default => odziv_napaka("Neznana akcija: $akcija", 400),
    };
}

function _modul_perun_info(array $p): array {
    return odziv_uspeh(["ime"=>"Perun","id"=>"perun","verzija"=>"1.0.0","opis"=>"Perun — slovansko božanstvo groma, pravice in nebesnih sil.","uporabnik"=>Modul_Bridge::uporabnik_pridobi()["ime"] ?? "Gost"], "Informacije");
}
function _modul_perun_domov(array $p): array {
    return odziv_uspeh(["sporocilo"=>"Pozdravljen v Perun!","cas"=>time()], "Domov");
}

if (basename($_SERVER["SCRIPT_FILENAME"] ?? "") === "modul.php" && !defined("SISTEM_OBSTAJA")) {
    $odziv = modul_perun_akcija($_REQUEST["akcija"] ?? "domov", $_REQUEST);
