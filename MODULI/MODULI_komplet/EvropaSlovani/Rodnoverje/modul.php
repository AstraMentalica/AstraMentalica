...
/**
 * MODUL: Rodnoverje
 * POT: MODULI/EvropaSlovani/Rodnoverje/modul.php
 * 📅 VERZIJA: v1.0.0 (2026-06-28)
 * 📰 NAMEN: Slovansko staroverstvo — predkrščanska slovanska duhovnost in bogovi.
 * 👤 AVTOR: Damir Šafarič
 */
...
function modul_rodnoverje_akcija(string $akcija, array $podatki = []): array {
    if (!Modul_Bridge::vloga_preveri("S0")) return odziv_napaka("Dostop zavrnjen", 403);
    return match($akcija) {
        "info"  => _modul_rodnoverje_info($podatki),
        "domov" => _modul_rodnoverje_domov($podatki),
        default => odziv_napaka("Neznana akcija: $akcija", 400),
    };
}

function _modul_rodnoverje_info(array $p): array {
    return odziv_uspeh(["ime"=>"Rodnoverje","id"=>"rodnoverje","verzija"=>"1.0.0","opis"=>"Slovansko staroverstvo — predkrščanska slovanska duhovnost in bogovi.","uporabnik"=>Modul_Bridge::uporabnik_pridobi()["ime"] ?? "Gost"], "Informacije");
}
function _modul_rodnoverje_domov(array $p): array {
    return odziv_uspeh(["sporocilo"=>"Pozdravljen v Rodnoverje!","cas"=>time()], "Domov");
}

if (basename($_SERVER["SCRIPT_FILENAME"] ?? "") === "modul.php" && !defined("SISTEM_OBSTAJA")) {
    $odziv = modul_rodnoverje_akcija($_REQUEST["akcija"] ?? "domov", $_REQUEST);
