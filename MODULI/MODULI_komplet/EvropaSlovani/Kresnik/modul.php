...
/**
 * MODUL: Kresnik
 * POT: MODULI/EvropaSlovani/Kresnik/modul.php
 * 📅 VERZIJA: v1.0.0 (2026-06-28)
 * 📰 NAMEN: Slovenska mistika — Kresnik, Kurent in slovensko duhovno izročilo.
 * 👤 AVTOR: Damir Šafarič
 */
...
function modul_kresnik_akcija(string $akcija, array $podatki = []): array {
    if (!Modul_Bridge::vloga_preveri("S0")) return odziv_napaka("Dostop zavrnjen", 403);
    return match($akcija) {
        "info"  => _modul_kresnik_info($podatki),
        "domov" => _modul_kresnik_domov($podatki),
        default => odziv_napaka("Neznana akcija: $akcija", 400),
    };
}

function _modul_kresnik_info(array $p): array {
    return odziv_uspeh(["ime"=>"Kresnik","id"=>"kresnik","verzija"=>"1.0.0","opis"=>"Slovenska mistika — Kresnik, Kurent in slovensko duhovno izročilo.","uporabnik"=>Modul_Bridge::uporabnik_pridobi()["ime"] ?? "Gost"], "Informacije");
}
function _modul_kresnik_domov(array $p): array {
    return odziv_uspeh(["sporocilo"=>"Pozdravljen v Kresnik!","cas"=>time()], "Domov");
}

if (basename($_SERVER["SCRIPT_FILENAME"] ?? "") === "modul.php" && !defined("SISTEM_OBSTAJA")) {
    $odziv = modul_kresnik_akcija($_REQUEST["akcija"] ?? "domov", $_REQUEST);
