...
/**
 * MODUL: Veles
 * POT: MODULI/EvropaSlovani/Veles/modul.php
 * 📅 VERZIJA: v1.0.0 (2026-06-28)
 * 📰 NAMEN: Veles — slovansko božanstvo podzemlja, narave in modrosti.
 * 👤 AVTOR: Damir Šafarič
 */
...
function modul_veles_akcija(string $akcija, array $podatki = []): array {
    if (!Modul_Bridge::vloga_preveri("S0")) return odziv_napaka("Dostop zavrnjen", 403);
    return match($akcija) {
        "info"  => _modul_veles_info($podatki),
        "domov" => _modul_veles_domov($podatki),
        default => odziv_napaka("Neznana akcija: $akcija", 400),
    };
}

function _modul_veles_info(array $p): array {
    return odziv_uspeh(["ime"=>"Veles","id"=>"veles","verzija"=>"1.0.0","opis"=>"Veles — slovansko božanstvo podzemlja, narave in modrosti.","uporabnik"=>Modul_Bridge::uporabnik_pridobi()["ime"] ?? "Gost"], "Informacije");
}
function _modul_veles_domov(array $p): array {
    return odziv_uspeh(["sporocilo"=>"Pozdravljen v Veles!","cas"=>time()], "Domov");
}

if (basename($_SERVER["SCRIPT_FILENAME"] ?? "") === "modul.php" && !defined("SISTEM_OBSTAJA")) {
    $odziv = modul_veles_akcija($_REQUEST["akcija"] ?? "domov", $_REQUEST);
