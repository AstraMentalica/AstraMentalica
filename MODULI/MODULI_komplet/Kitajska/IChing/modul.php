...
/**
 * MODUL: IChing
 * POT: MODULI/Kitajska/IChing/modul.php
 * 📅 VERZIJA: v1.0.0 (2026-06-28)
 * 📰 NAMEN: Knjiga sprememb — I Ching, trigrami, heksagrami in kitajska prerokba.
 * 👤 AVTOR: Damir Šafarič
 */
...
function modul_iching_akcija(string $akcija, array $podatki = []): array {
    if (!Modul_Bridge::vloga_preveri("S0")) return odziv_napaka("Dostop zavrnjen", 403);
    return match($akcija) {
        "info"  => _modul_iching_info($podatki),
        "domov" => _modul_iching_domov($podatki),
        default => odziv_napaka("Neznana akcija: $akcija", 400),
    };
}

function _modul_iching_info(array $p): array {
    return odziv_uspeh(["ime"=>"IChing","id"=>"iching","verzija"=>"1.0.0","opis"=>"Knjiga sprememb — I Ching, trigrami, heksagrami in kitajska prerokba.","uporabnik"=>Modul_Bridge::uporabnik_pridobi()["ime"] ?? "Gost"], "Informacije");
}
function _modul_iching_domov(array $p): array {
    return odziv_uspeh(["sporocilo"=>"Pozdravljen v IChing!","cas"=>time()], "Domov");
}

if (basename($_SERVER["SCRIPT_FILENAME"] ?? "") === "modul.php" && !defined("SISTEM_OBSTAJA")) {
    $odziv = modul_iching_akcija($_REQUEST["akcija"] ?? "domov", $_REQUEST);
