...
/**
 * MODUL: Tao
 * POT: MODULI/Kitajska/Tao/modul.php
 * 📅 VERZIJA: v1.0.0 (2026-06-28)
 * 📰 NAMEN: Pot Tao — taoizem, wu wei, naravna pot in ravnovesje nasprotij.
 * 👤 AVTOR: Damir Šafarič
 */
...
function modul_tao_akcija(string $akcija, array $podatki = []): array {
    if (!Modul_Bridge::vloga_preveri("S0")) return odziv_napaka("Dostop zavrnjen", 403);
    return match($akcija) {
        "info"  => _modul_tao_info($podatki),
        "domov" => _modul_tao_domov($podatki),
        default => odziv_napaka("Neznana akcija: $akcija", 400),
    };
}

function _modul_tao_info(array $p): array {
    return odziv_uspeh(["ime"=>"Tao","id"=>"tao","verzija"=>"1.0.0","opis"=>"Pot Tao — taoizem, wu wei, naravna pot in ravnovesje nasprotij.","uporabnik"=>Modul_Bridge::uporabnik_pridobi()["ime"] ?? "Gost"], "Informacije");
}
function _modul_tao_domov(array $p): array {
    return odziv_uspeh(["sporocilo"=>"Pozdravljen v Tao!","cas"=>time()], "Domov");
}

if (basename($_SERVER["SCRIPT_FILENAME"] ?? "") === "modul.php" && !defined("SISTEM_OBSTAJA")) {
    $odziv = modul_tao_akcija($_REQUEST["akcija"] ?? "domov", $_REQUEST);
