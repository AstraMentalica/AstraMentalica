...
/**
 * MODUL: Dhyana
 * POT: MODULI/Indija/Dhyana/modul.php
 * 📅 VERZIJA: v1.0.0 (2026-06-28)
 * 📰 NAMEN: Meditacija — vipassana, transcendentalna in vedantska meditacija.
 * 👤 AVTOR: Damir Šafarič
 */
...
function modul_dhyana_akcija(string $akcija, array $podatki = []): array {
    if (!Modul_Bridge::vloga_preveri("S0")) return odziv_napaka("Dostop zavrnjen", 403);
    return match($akcija) {
        "info"  => _modul_dhyana_info($podatki),
        "domov" => _modul_dhyana_domov($podatki),
        default => odziv_napaka("Neznana akcija: $akcija", 400),
    };
}

function _modul_dhyana_info(array $p): array {
    return odziv_uspeh(["ime"=>"Dhyana","id"=>"dhyana","verzija"=>"1.0.0","opis"=>"Meditacija — vipassana, transcendentalna in vedantska meditacija.","uporabnik"=>Modul_Bridge::uporabnik_pridobi()["ime"] ?? "Gost"], "Informacije");
}
function _modul_dhyana_domov(array $p): array {
    return odziv_uspeh(["sporocilo"=>"Pozdravljen v Dhyana!","cas"=>time()], "Domov");
}

if (basename($_SERVER["SCRIPT_FILENAME"] ?? "") === "modul.php" && !defined("SISTEM_OBSTAJA")) {
    $odziv = modul_dhyana_akcija($_REQUEST["akcija"] ?? "domov", $_REQUEST);
