...
/**
 * MODUL: Pranayama
 * POT: MODULI/Indija/Pranayama/modul.php
 * 📅 VERZIJA: v1.0.0 (2026-06-28)
 * 📰 NAMEN: Dihalne tehnike — pranajama, kontrola diha in vitalne energije.
 * 👤 AVTOR: Damir Šafarič
 */
...
function modul_pranayama_akcija(string $akcija, array $podatki = []): array {
    if (!Modul_Bridge::vloga_preveri("S0")) return odziv_napaka("Dostop zavrnjen", 403);
    return match($akcija) {
        "info"  => _modul_pranayama_info($podatki),
        "domov" => _modul_pranayama_domov($podatki),
        default => odziv_napaka("Neznana akcija: $akcija", 400),
    };
}

function _modul_pranayama_info(array $p): array {
    return odziv_uspeh(["ime"=>"Pranayama","id"=>"pranayama","verzija"=>"1.0.0","opis"=>"Dihalne tehnike — pranajama, kontrola diha in vitalne energije.","uporabnik"=>Modul_Bridge::uporabnik_pridobi()["ime"] ?? "Gost"], "Informacije");
}
function _modul_pranayama_domov(array $p): array {
    return odziv_uspeh(["sporocilo"=>"Pozdravljen v Pranayama!","cas"=>time()], "Domov");
}

if (basename($_SERVER["SCRIPT_FILENAME"] ?? "") === "modul.php" && !defined("SISTEM_OBSTAJA")) {
    $odziv = modul_pranayama_akcija($_REQUEST["akcija"] ?? "domov", $_REQUEST);
