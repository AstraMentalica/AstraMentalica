...
/**
 * MODUL: Angelarium
 * POT: MODULI/HebrejskaMistika/Angelarium/modul.php
 * 📅 VERZIJA: v1.0.0 (2026-06-28)
 * 📰 NAMEN: Svet angelov — angelska sporočila, varstvo in komunikacija z angeli.
 * 👤 AVTOR: Damir Šafarič
 */
...
function modul_angelarium_akcija(string $akcija, array $podatki = []): array {
    if (!Modul_Bridge::vloga_preveri("S0")) return odziv_napaka("Dostop zavrnjen", 403);
    return match($akcija) {
        "info"  => _modul_angelarium_info($podatki),
        "domov" => _modul_angelarium_domov($podatki),
        default => odziv_napaka("Neznana akcija: $akcija", 400),
    };
}

function _modul_angelarium_info(array $p): array {
    return odziv_uspeh(["ime"=>"Angelarium","id"=>"angelarium","verzija"=>"1.0.0","opis"=>"Svet angelov — angelska sporočila, varstvo in komunikacija z angeli.","uporabnik"=>Modul_Bridge::uporabnik_pridobi()["ime"] ?? "Gost"], "Informacije");
}
function _modul_angelarium_domov(array $p): array {
    return odziv_uspeh(["sporocilo"=>"Pozdravljen v Angelarium!","cas"=>time()], "Domov");
}

if (basename($_SERVER["SCRIPT_FILENAME"] ?? "") === "modul.php" && !defined("SISTEM_OBSTAJA")) {
    $odziv = modul_angelarium_akcija($_REQUEST["akcija"] ?? "domov", $_REQUEST);
