...
/**
 * MODUL: Malakhim
 * POT: MODULI/HebrejskaMistika/Malakhim/modul.php
 * 📅 VERZIJA: v1.0.0 (2026-06-28)
 * 📰 NAMEN: Angelske hierarhije — angelski redi, arhangeli in nebeška hierarhija.
 * 👤 AVTOR: Damir Šafarič
 */
...
function modul_malakhim_akcija(string $akcija, array $podatki = []): array {
    if (!Modul_Bridge::vloga_preveri("S0")) return odziv_napaka("Dostop zavrnjen", 403);
    return match($akcija) {
        "info"  => _modul_malakhim_info($podatki),
        "domov" => _modul_malakhim_domov($podatki),
        default => odziv_napaka("Neznana akcija: $akcija", 400),
    };
}

function _modul_malakhim_info(array $p): array {
    return odziv_uspeh(["ime"=>"Malakhim","id"=>"malakhim","verzija"=>"1.0.0","opis"=>"Angelske hierarhije — angelski redi, arhangeli in nebeška hierarhija.","uporabnik"=>Modul_Bridge::uporabnik_pridobi()["ime"] ?? "Gost"], "Informacije");
}
function _modul_malakhim_domov(array $p): array {
    return odziv_uspeh(["sporocilo"=>"Pozdravljen v Malakhim!","cas"=>time()], "Domov");
}

if (basename($_SERVER["SCRIPT_FILENAME"] ?? "") === "modul.php" && !defined("SISTEM_OBSTAJA")) {
    $odziv = modul_malakhim_akcija($_REQUEST["akcija"] ?? "domov", $_REQUEST);
