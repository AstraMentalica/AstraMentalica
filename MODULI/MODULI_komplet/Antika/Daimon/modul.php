...
/**
 * MODUL: Daimon
 * POT: MODULI/Antika/Daimon/modul.php
 * 📅 VERZIJA: v1.0.0 (2026-06-28)
 * 📰 NAMEN: Duhovni vodniki — grška tradicija osebnega duha in notranjega vodnika.
 * 👤 AVTOR: Damir Šafarič
 */
...
function modul_daimon_akcija(string $akcija, array $podatki = []): array {
    if (!Modul_Bridge::vloga_preveri("S0")) return odziv_napaka("Dostop zavrnjen", 403);
    return match($akcija) {
        "info"  => _modul_daimon_info($podatki),
        "domov" => _modul_daimon_domov($podatki),
        default => odziv_napaka("Neznana akcija: $akcija", 400),
    };
}

function _modul_daimon_info(array $p): array {
    return odziv_uspeh(["ime"=>"Daimon","id"=>"daimon","verzija"=>"1.0.0","opis"=>"Duhovni vodniki — grška tradicija osebnega duha in notranjega vodnika.","uporabnik"=>Modul_Bridge::uporabnik_pridobi()["ime"] ?? "Gost"], "Informacije");
}
function _modul_daimon_domov(array $p): array {
    return odziv_uspeh(["sporocilo"=>"Pozdravljen v Daimon!","cas"=>time()], "Domov");
}

if (basename($_SERVER["SCRIPT_FILENAME"] ?? "") === "modul.php" && !defined("SISTEM_OBSTAJA")) {
    $odziv = modul_daimon_akcija($_REQUEST["akcija"] ?? "domov", $_REQUEST);
