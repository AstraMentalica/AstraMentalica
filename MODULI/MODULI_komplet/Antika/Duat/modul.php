...
/**
 * MODUL: Duat
 * POT: MODULI/Antika/Duat/modul.php
 * 📅 VERZIJA: v1.0.0 (2026-06-28)
 * 📰 NAMEN: Pot skozi podzemlje — egipčanska knjiga mrtvih, Amduat in pot duše.
 * 👤 AVTOR: Damir Šafarič
 */
...
function modul_duat_akcija(string $akcija, array $podatki = []): array {
    if (!Modul_Bridge::vloga_preveri("S0")) return odziv_napaka("Dostop zavrnjen", 403);
    return match($akcija) {
        "info"  => _modul_duat_info($podatki),
        "domov" => _modul_duat_domov($podatki),
        default => odziv_napaka("Neznana akcija: $akcija", 400),
    };
}

function _modul_duat_info(array $p): array {
    return odziv_uspeh(["ime"=>"Duat","id"=>"duat","verzija"=>"1.0.0","opis"=>"Pot skozi podzemlje — egipčanska knjiga mrtvih, Amduat in pot duše.","uporabnik"=>Modul_Bridge::uporabnik_pridobi()["ime"] ?? "Gost"], "Informacije");
}
function _modul_duat_domov(array $p): array {
    return odziv_uspeh(["sporocilo"=>"Pozdravljen v Duat!","cas"=>time()], "Domov");
}

if (basename($_SERVER["SCRIPT_FILENAME"] ?? "") === "modul.php" && !defined("SISTEM_OBSTAJA")) {
    $odziv = modul_duat_akcija($_REQUEST["akcija"] ?? "domov", $_REQUEST);
