...
/**
 * MODUL: Labyrinthos
 * POT: MODULI/Antika/Labyrinthos/modul.php
 * 📅 VERZIJA: v1.0.0 (2026-06-28)
 * 📰 NAMEN: Labirint zavesti — grški labirint kot metafora duhovne poti.
 * 👤 AVTOR: Damir Šafarič
 */
...
function modul_labyrinthos_akcija(string $akcija, array $podatki = []): array {
    if (!Modul_Bridge::vloga_preveri("S0")) return odziv_napaka("Dostop zavrnjen", 403);
    return match($akcija) {
        "info"  => _modul_labyrinthos_info($podatki),
        "domov" => _modul_labyrinthos_domov($podatki),
        default => odziv_napaka("Neznana akcija: $akcija", 400),
    };
}

function _modul_labyrinthos_info(array $p): array {
    return odziv_uspeh(["ime"=>"Labyrinthos","id"=>"labyrinthos","verzija"=>"1.0.0","opis"=>"Labirint zavesti — grški labirint kot metafora duhovne poti.","uporabnik"=>Modul_Bridge::uporabnik_pridobi()["ime"] ?? "Gost"], "Informacije");
}
function _modul_labyrinthos_domov(array $p): array {
    return odziv_uspeh(["sporocilo"=>"Pozdravljen v Labyrinthos!","cas"=>time()], "Domov");
}

if (basename($_SERVER["SCRIPT_FILENAME"] ?? "") === "modul.php" && !defined("SISTEM_OBSTAJA")) {
    $odziv = modul_labyrinthos_akcija($_REQUEST["akcija"] ?? "domov", $_REQUEST);
