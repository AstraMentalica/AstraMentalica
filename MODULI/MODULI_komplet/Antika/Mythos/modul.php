...
/**
 * MODUL: Mythos
 * POT: MODULI/Antika/Mythos/modul.php
 * 📅 VERZIJA: v1.0.0 (2026-06-28)
 * 📰 NAMEN: Grška mitologija — heroji, bogovi in kozmogonija stare Grčije.
 * 👤 AVTOR: Damir Šafarič
 */
...
function modul_mythos_akcija(string $akcija, array $podatki = []): array {
    if (!Modul_Bridge::vloga_preveri("S0")) return odziv_napaka("Dostop zavrnjen", 403);
    return match($akcija) {
        "info"  => _modul_mythos_info($podatki),
        "domov" => _modul_mythos_domov($podatki),
        default => odziv_napaka("Neznana akcija: $akcija", 400),
    };
}

function _modul_mythos_info(array $p): array {
    return odziv_uspeh(["ime"=>"Mythos","id"=>"mythos","verzija"=>"1.0.0","opis"=>"Grška mitologija — heroji, bogovi in kozmogonija stare Grčije.","uporabnik"=>Modul_Bridge::uporabnik_pridobi()["ime"] ?? "Gost"], "Informacije");
}
function _modul_mythos_domov(array $p): array {
    return odziv_uspeh(["sporocilo"=>"Pozdravljen v Mythos!","cas"=>time()], "Domov");
}

if (basename($_SERVER["SCRIPT_FILENAME"] ?? "") === "modul.php" && !defined("SISTEM_OBSTAJA")) {
    $odziv = modul_mythos_akcija($_REQUEST["akcija"] ?? "domov", $_REQUEST);
