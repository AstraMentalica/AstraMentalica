 ...
 * 📰 NAMEN: Egipčanska božanstva — panteон bogov in boginj starodavnega Egipta.
 * 👤 AVTOR: Damir Šafarič
 */
...
function _modul_neteru_info(array $p): array {
    return odziv_uspeh(["ime"=>"Neteru","id"=>"neteru","verzija"=>"1.0.0","opis"=>"Egipčanska božanstva — panteон bogov in boginj starodavnega Egipta.","uporabnik"=>Modul_Bridge::uporabnik_pridobi()["ime"] ?? "Gost"], "Informacije");
}
function _modul_neteru_domov(array $p): array {
    return odziv_uspeh(["sporocilo"=>"Pozdravljen v Neteru!","cas"=>time()], "Domov");
}

if (basename($_SERVER["SCRIPT_FILENAME"] ?? "") === "modul.php" && !defined("SISTEM_OBSTAJA")) {
    $odziv = modul_neteru_akcija($_REQUEST["akcija"] ?? "domov", $_REQUEST);
