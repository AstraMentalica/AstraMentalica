...
/**
 * MODUL: Sufica
 * POT: MODULI/Prihodnost/Sufica/modul.php
 * 📅 VERZIJA: v1.0.0 (2026-06-28)
 * 📰 NAMEN: Sufizem — islamska mistika, zikr, derviški ples in Rumi.
 * 👤 AVTOR: Damir Šafarič
 */
...
function modul_sufica_akcija(string $akcija, array $podatki = []): array {
    if (!Modul_Bridge::vloga_preveri("S0")) return odziv_napaka("Dostop zavrnjen", 403);
    return match($akcija) {
        "info"  => _modul_sufica_info($podatki),
        "domov" => _modul_sufica_domov($podatki),
        default => odziv_napaka("Neznana akcija: $akcija", 400),
    };
}

function _modul_sufica_info(array $p): array {
    return odziv_uspeh(["ime"=>"Sufica","id"=>"sufica","verzija"=>"1.0.0","opis"=>"Sufizem — islamska mistika, zikr, derviški ples in Rumi.","uporabnik"=>Modul_Bridge::uporabnik_pridobi()["ime"] ?? "Gost"], "Informacije");
}
function _modul_sufica_domov(array $p): array {
    return odziv_uspeh(["sporocilo"=>"Pozdravljen v Sufica!","cas"=>time()], "Domov");
}

if (basename($_SERVER["SCRIPT_FILENAME"] ?? "") === "modul.php" && !defined("SISTEM_OBSTAJA")) {
    $odziv = modul_sufica_akcija($_REQUEST["akcija"] ?? "domov", $_REQUEST);
