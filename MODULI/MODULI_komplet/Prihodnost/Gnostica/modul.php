...
/**
 * MODUL: Gnostica
 * POT: MODULI/Prihodnost/Gnostica/modul.php
 * 📅 VERZIJA: v1.0.0 (2026-06-28)
 * 📰 NAMEN: Gnosticizem — gnostični miti, Demiurg in iskanje božanske iskre.
 * 👤 AVTOR: Damir Šafarič
 */
...
function modul_gnostica_akcija(string $akcija, array $podatki = []): array {
    if (!Modul_Bridge::vloga_preveri("S1")) return odziv_napaka("Dostop zavrnjen", 403);
    return match($akcija) {
        "info"  => _modul_gnostica_info($podatki),
        "domov" => _modul_gnostica_domov($podatki),
        default => odziv_napaka("Neznana akcija: $akcija", 400),
    };
}

function _modul_gnostica_info(array $p): array {
    return odziv_uspeh(["ime"=>"Gnostica","id"=>"gnostica","verzija"=>"1.0.0","opis"=>"Gnosticizem — gnostični miti, Demiurg in iskanje božanske iskre.","uporabnik"=>Modul_Bridge::uporabnik_pridobi()["ime"] ?? "Gost"], "Informacije");
}
function _modul_gnostica_domov(array $p): array {
    return odziv_uspeh(["sporocilo"=>"Pozdravljen v Gnostica!","cas"=>time()], "Domov");
}

if (basename($_SERVER["SCRIPT_FILENAME"] ?? "") === "modul.php" && !defined("SISTEM_OBSTAJA")) {
    $odziv = modul_gnostica_akcija($_REQUEST["akcija"] ?? "domov", $_REQUEST);
