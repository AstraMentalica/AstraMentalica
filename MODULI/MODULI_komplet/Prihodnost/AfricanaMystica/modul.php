<?php
/**
 * MODUL: AfricanaMystica
 * POT: MODULI/Prihodnost/AfricanaMystica/modul.php
 * 📅 VERZIJA: v1.0.0 (2026-06-28)
 * 📰 NAMEN: Afriške tradicije — Yoruba, Voodoo, Ubuntu in afriška duhovna modrost.
 * 👤 AVTOR: Damir Šafarič
 */
...
function modul_africanamystica_akcija(string $akcija, array $podatki = []): array {
    if (!Modul_Bridge::vloga_preveri("S0")) return odziv_napaka("Dostop zavrnjen", 403);
    return match($akcija) {
        "info"  => _modul_africanamystica_info($podatki),
        "domov" => _modul_africanamystica_domov($podatki),
        default => odziv_napaka("Neznana akcija: $akcija", 400),
    };
}

function _modul_africanamystica_info(array $p): array {
    return odziv_uspeh(["ime"=>"AfricanaMystica","id"=>"africanamystica","verzija"=>"1.0.0","opis"=>"Afriške tradicije — Yoruba, Voodoo, Ubuntu in afriška duhovna modrost.","uporabnik"=>Modul_Bridge::uporabnik_pridobi()["ime"] ?? "Gost"], "Informacije");
}
function _modul_africanamystica_domov(array $p): array {
    return odziv_uspeh(["sporocilo"=>"Pozdravljen v AfricanaMystica!","cas"=>time()], "Domov");
}

if (basename($_SERVER["SCRIPT_FILENAME"] ?? "") === "modul.php" && !defined("SISTEM_OBSTAJA")) {
    $odziv = modul_africanamystica_akcija($_REQUEST["akcija"] ?? "domov", $_REQUEST);
