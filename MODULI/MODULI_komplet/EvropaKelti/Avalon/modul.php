...
/**
 * MODUL: Avalon
 * POT: MODULI/EvropaKelti/Avalon/modul.php
 * 📅 VERZIJA: v1.0.0 (2026-06-28)
 * 📰 NAMEN: Keltski misteriji — Avalon, Arturjanska legenda in keltski mistični svet.
 * 👤 AVTOR: Damir Šafarič
 */
...
function modul_avalon_akcija(string $akcija, array $podatki = []): array {
    if (!Modul_Bridge::vloga_preveri("S0")) return odziv_napaka("Dostop zavrnjen", 403);
    return match($akcija) {
        "info"  => _modul_avalon_info($podatki),
        "domov" => _modul_avalon_domov($podatki),
        default => odziv_napaka("Neznana akcija: $akcija", 400),
    };
}

function _modul_avalon_info(array $p): array {
    return odziv_uspeh(["ime"=>"Avalon","id"=>"avalon","verzija"=>"1.0.0","opis"=>"Keltski misteriji — Avalon, Arturjanska legenda in keltski mistični svet.","uporabnik"=>Modul_Bridge::uporabnik_pridobi()["ime"] ?? "Gost"], "Informacije");
}
function _modul_avalon_domov(array $p): array {
    return odziv_uspeh(["sporocilo"=>"Pozdravljen v Avalon!","cas"=>time()], "Domov");
}

if (basename($_SERVER["SCRIPT_FILENAME"] ?? "") === "modul.php" && !defined("SISTEM_OBSTAJA")) {
    $odziv = modul_avalon_akcija($_REQUEST["akcija"] ?? "domov", $_REQUEST);
