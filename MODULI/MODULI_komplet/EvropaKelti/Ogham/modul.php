...
/**
 * MODUL: Ogham
 * POT: MODULI/EvropaKelti/Ogham/modul.php
 * 📅 VERZIJA: v1.0.0 (2026-06-28)
 * 📰 NAMEN: Keltska pisava — oghamska abeceda in njena duhovna simbolika.
 * 👤 AVTOR: Damir Šafarič
 */
...
function modul_ogham_akcija(string $akcija, array $podatki = []): array {
    if (!Modul_Bridge::vloga_preveri("S0")) return odziv_napaka("Dostop zavrnjen", 403);
    return match($akcija) {
        "info"  => _modul_ogham_info($podatki),
        "domov" => _modul_ogham_domov($podatki),
        default => odziv_napaka("Neznana akcija: $akcija", 400),
    };
}

function _modul_ogham_info(array $p): array {
    return odziv_uspeh(["ime"=>"Ogham","id"=>"ogham","verzija"=>"1.0.0","opis"=>"Keltska pisava — oghamska abeceda in njena duhovna simbolika.","uporabnik"=>Modul_Bridge::uporabnik_pridobi()["ime"] ?? "Gost"], "Informacije");
}
function _modul_ogham_domov(array $p): array {
    return odziv_uspeh(["sporocilo"=>"Pozdravljen v Ogham!","cas"=>time()], "Domov");
}

if (basename($_SERVER["SCRIPT_FILENAME"] ?? "") === "modul.php" && !defined("SISTEM_OBSTAJA")) {
    $odziv = modul_ogham_akcija($_REQUEST["akcija"] ?? "domov", $_REQUEST);
