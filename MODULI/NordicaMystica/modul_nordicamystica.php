...
require_once __DIR__ . '/modul_nordicamystica_funkcije.php';
require_once __DIR__ . '/modul_nordicamystica_pravila.php';
require_once __DIR__ . '/modul_nordicamystica_jsonbaza.php';

class ModulNordicaMystica {
    private $db;
    private $config;

    public function __construct() {
        $this->db = new ModulNordicaMysticaJsonBaza();
        $this->config = $this->db->pridobiVse();
    }

    public function pridobiOsnovnePodatke() {
        return [
            'ime' => $this->config['ime'] ?? 'NordicaMystica',
    ...
    public function pridobiVsebino($param = []) {
        $html = \"<div class='modul-nordicamystica'>\" . htmlspecialchars($this->config['opis'] ?? '') . \"</div>\";
        return ['naslov' => $this->config['opis'] ?? 'NordicaMystica', 'vsebina' => $html];
    }
