...
/**
 * MODUL: Occultum
 * POT: MODULI/Occultum/modul.php
 * VERZIJA: 1.0.0 (24.6.2026)
 *
 * 🏛️ NIVO: MODUL
 *
 * 📰 NAMEN:
 *     Standardna vstopna točka za Occultum modul
 *     Avtomatsko generirano
 *
 * 🔧 JAVNE FUNKCIJE:
 *     - modul_occultum_akcija(string $akcija, array $podatki): array
 *
 * 👤 AVTOR:
...
function modul_occultum_akcija(string $akcija, array $podatki = []): array {
    ...
    return match($akcija) {
        'info'  => _modul_occultum_akcija_info($podatki),
        'domov' => _modul_occultum_akcija_domov($podatki),
...
function _modul_occultum_akcija_info(array $podatki): array {
    return odziv_uspeh([
        'ime'       => 'Occultum',
        'id'        => 'occultum',
        'verzija'   => '1.0.0',
...
function _modul_occultum_akcija_domov(array $podatki): array {
    return odziv_uspeh([
        'sporocilo' => 'Pozdravljen v modulu Occultum!',
        'cas'       => time(),
    ], 'Domov');
}
...
if (basename($_SERVER['SCRIPT_FILENAME'] ?? '') === 'modul.php' && !defined('SISTEM_OBSTAJA')) {
    ...
    $podatki = $_REQUEST;
    $odziv   = modul_occultum_akcija($akcija, $podatki);
