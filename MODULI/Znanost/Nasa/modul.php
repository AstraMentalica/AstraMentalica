 ...
 *
 * 📰 NAMEN:
 *     NASA senzorji — sončeva aktivnost (NASA DONKI), geomagnetna aktivnost (NOAA SWPC).
 *
 * 🔧 JAVNE FUNKCIJE:
 *     - modul_nasa_akcija(string $akcija, array $podatki): array
 *
 * 📡 ODVISNOSTI:
 *     - Modul_Bridge
 *
...
function _modul_nasa_info(array $podatki): array {
    ...
    return odziv_uspeh([
        ...
        'kategorija'=> 'Znanost',
        'verzija'   => '1.0.0',
        'opis'      => 'NASA senzorji — sončeva aktivnost (NASA DONKI), geomagnetna aktivnost (NOAA SWPC).',
        'uporabnik' => $uporabnik['ime'] ?? 'Gost',
    ], 'Informacije o modulu');
}

function _modul_nasa_domov(array $podatki): array {
    return odziv_uspeh([
        'sporocilo' => 'Pozdravljen v modulu Nasa!',
        'cas'       => time(),
    ], 'Domov');
}
...
if (basename($_SERVER['SCRIPT_FILENAME'] ?? '') === 'modul.php' && !defined('SISTEM_OBSTAJA')) {
    ...
    $odziv   = modul_nasa_akcija($akcija, $_REQUEST);
