...
const Universe = (function() {
    ...
    let cameraTheta = 0.8, cameraPhi = 0.4, cameraRadius = 28;
    let stars = [];
    let raycaster;
    let mouse;
    ...
    function onMouseDown(e) {
        ...
        renderer.domElement._dragStart = {x: e.clientX, y: e.clientY};
    }
    
    function onMouseMove(e) {
        if (!renderer.domElement._dragStart) return;
        const dx = e.clientX - lastMouse.x;
        const dy = e.clientY - lastMouse.y;
        const totalDist = Math.abs(e.clientX - renderer.domElement._dragStart.x) + 
                          Math.abs(e.clientY - renderer.domElement._dragStart.y);
        if (totalDist > 5) isDragging = true;
        cameraTheta -= dx * 0.005;
        cameraPhi = Math.max(-0.5, Math.min(1.2, cameraPhi - dy * 0.005));
        lastMouse = {x: e.clientX, y: e.clientY};
        updateCameraPosition();
    }
    
    function onMouseUp(e) {
        renderer.domElement._dragStart = null;
        renderer.domElement.style.cursor = 'grab';
    }
    
    function onMouseWheel(e) {
        e.preventDefault();
        cameraRadius = Math.max(5, Math.min(80, cameraRadius + e.deltaY * 0.05));
        updateCameraPosition();
    }
    ...
    function onTouchMove(e) {
        ...
        if (e.touches.length === 1 && lastTouches?.length === 1) {
            ...
            cameraPhi = Math.max(-0.5, Math.min(1.2, cameraPhi - dy * 0.007));
            isDragging = true;
            updateCameraPosition();
        } else if (e.touches.length === 2 && lastTouches?.length === 2) {
            const d1 = Math.hypot(lastTouches[0].clientX - lastTouches[1].clientX,
                                  lastTouches[0].clientY - lastTouches[1].clientY);
            const d2 = Math.hypot(e.touches[0].clientX - e.touches[1].clientX,
                                  e.touches[0].clientY - e.touches[1].clientY);
            cameraRadius = Math.max(5, Math.min(80, cameraRadius - (d2 - d1) * 0.1));
            updateCameraPosition();
        }
        lastTouches = e.touches;
    }
    ...
    function onMouseClick(event) {
        ...
        // Vključi tudi Codex
        const codexObj = scene.getObjectByName('codex_mesh');
        if (codexObj) planetMeshes.push(codexObj);
        
        const intersects = raycaster.intersectObjects(planetMeshes, true);
        
        if (intersects.length > 0) {
            let obj = intersects[0].object;
            // Pojdi na planet (ne child objekte)
            while (obj.parent && !obj.userData.id) obj = obj.parent;
            if (obj.userData.id) showPlanetInfo(obj);
        } else {
            closeInfo();
        }
    }
