<?php
/**
 * avto.php – AVTOMATIZIRAN PREGLED + SPOMIN
 * ============================================================
 * Spomin: shrani zadnjo analizo in popravke za vsako mapo.
 * Ob ponovnem zagonu ponudi "Nadaljuj" (uporabi shranjeno) ali "Osveži".
 * ============================================================
 */
$ROOT = __DIR__;
$API_KEY = 'sk-da7741c3f9c04ce69c64075d8ead0bca';
$SPOMIN_MAPA = $ROOT . '/spomin';

if (!is_dir($SPOMIN_MAPA)) mkdir($SPOMIN_MAPA, 0755, true);

// ── POMOŽNE FUNKCIJE ZA SPOMIN ───────────────────────────────
function shraniSpomin(string $mapaIme, array $podatki): void {
    global $SPOMIN_MAPA;
    $file = $SPOMIN_MAPA . '/' . md5($mapaIme) . '.json';
    $podatki['zadnji_posodobljen'] = date('Y-m-d H:i:s');
    file_put_contents($file, json_encode($podatki, JSON_PRETTY_PRINT | JSON_UNESCAPED_UNICODE));
}

function naloziSpomin(string $mapaIme): ?array {
    global $SPOMIN_MAPA;
    $file = $SPOMIN_MAPA . '/' . md5($mapaIme) . '.json';
...
function izbrisiSpomin(string $mapaIme): void {
    global $SPOMIN_MAPA;
    $file = $SPOMIN_MAPA . '/' . md5($mapaIme) . '.json';
    ...
    try {
        switch ($akcija) {
            case 'preberi':
                ...
                // Shrani v spomin (samo osnovne podatke, brez analize)
                shraniSpomin($mapa, ['mapa' => $mapa, 'datoteke' => $datoteke, 'stevilo' => count($datoteke)]);
            ...
            case 'analiziraj':
                ...
                // Shrani analizo v spomin
                $spomin = naloziSpomin($mapa) ?: [];
                $spomin['analiza'] = $odg;
                $spomin['zadnja_analiza'] = date('Y-m-d H:i:s');
                shraniSpomin($mapa, $spomin);
            ...
            case 'popravi':
                ...
                if (!$json) {
                    ...
                    if (!empty($matches)) $json = json_decode($matches[0], true);
                }
                if (!$json) throw new Exception("Koder ni vrnil veljavnega JSON.");
                
                // Shrani popravke v spomin
                $spomin = naloziSpomin($mapa) ?: [];
                $spomin['popravki'] = $json;
                $spomin['zadnji_popravki'] = date('Y-m-d H:i:s');
                shraniSpomin($mapa, $spomin);
            ...
            case 'shrani':
                ...
                // Označi, da je bilo shranjeno
                $spomin = naloziSpomin($mapa) ?: [];
                $spomin['shranjeno'] = date('Y-m-d H:i:s');
                shraniSpomin($mapa, $spomin);
            ...
            case 'nalozi_spomin':
                $mapa = $_POST['mapa'] ?? '';
                $spomin = naloziSpomin($mapa);
                echo json_encode(['uspeh' => true, 'spomin' => $spomin]);
                exit;
...
<!DOCTYPE html>
<html>
<head><meta charset="UTF-8"><title>Avto pregled + Spomin</title>
<style>
...
.spomin-info{background:#0a0d14;border:1px solid #2a2f42;border-radius:6px;padding:8px 12px;margin-bottom:10px;font-size:12px;color:#6b7494;display:flex;justify-content:space-between;align-items:center;flex-wrap:wrap}
.spomin-info strong{color:#d4c5a9}
</style>
</head>
<body>
<div class="box">
<h1>🧠 Avto pregled + Spomin</h1>
<div class="sub">1. Izberi mapo → 2. Preglej → 3. Popravi → 4. Shrani</div>
...
<div class="row">
    ...
    <button class="gumb gumb-s" onclick="prikaziSpomin()">🧠 Spomin</button>
    <button class="gumb gumb-s" onclick="izbrisiSpomin()">🗑 Izbriši spomin</button>
</div>

<div id="spomin-info" class="spomin-info" style="display:none;"></div>
...
async function preglej() {
    trenutnaMapa = document.getElementById('mapa').value.trim() || '';
    document.getElementById('status').textContent = '⏳ Berem datoteke...';
    document.getElementById('izpis').textContent = '⏳ Berem datoteke iz ' + (trenutnaMapa || 'korena') + '...';
    
    try {
        // 1. Preberi datoteke
        const fd1 = new FormData();
        fd1.append('akcija', 'preberi');
        fd1.append('mapa', trenutnaMapa);
...
async function prikaziSpomin() {
    const mapa = document.getElementById('mapa').value.trim() || '';
    try {
        const fd = new FormData();
        fd.append('akcija', 'nalozi_spomin');
        fd.append('mapa', mapa);
        const r = await fetch(location.href, {method:'POST', body:fd});
        const d = await r.json();
        if (!d.uspeh || !d.spomin) {
            document.getElementById('spomin-info').style.display = 'none';
            return;
        }
        const s = d.spomin;
        let html = `<strong>🧠 Spomin za ${mapa || 'koren'}</strong> `;
        if (s.stevilo) html += `| 📂 ${s.stevilo} datotek`;
        if (s.zadnja_analiza) html += ` | 📋 analiza: ${s.zadnja_analiza}`;
        if (s.zadnji_popravki) html += ` | ✏️ popravki: ${s.zadnji_popravki}`;
        if (s.shranjeno) html += ` | 💾 shranjeno: ${s.shranjeno}`;
        document.getElementById('spomin-info').innerHTML = html;
        document.getElementById('spomin-info').style.display = 'flex';
    } catch(e) {
        // ignoriraj
    }
}

async function izbrisiSpomin() {
    if (!confirm('Izbriši shranjen spomin za to mapo?')) return;
    const mapa = document.getElementById('mapa').value.trim() || '';
    try {
        const fd = new FormData();
        fd.append('akcija', 'izbrisi_spomin');
        fd.append('mapa', mapa);
        await fetch(location.href, {method:'POST', body:fd});
        document.getElementById('spomin-info').style.display = 'none';
        document.getElementById('status').textContent = '🗑 Spomin izbrisan.';
    } catch(e) {}
}

// Ob nalaganju strani preveri spomin
