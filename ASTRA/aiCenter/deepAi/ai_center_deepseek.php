<?php
/**
 * DeepSeek AI Center - Multi-Agent, Multi-API
 */

require_once '../includes/ai_deepseek.php';
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    elseif ($akcija === 'pogovor_med_agenti') {
        ...
        if ($result['success']) {
            $sporocilo = 'Pogovor med agentoma končan';
            $rezultat = $result;
        } else {
            $napaka = $result['error'];
        }
...
$agenti = $ai->db->query("SELECT * FROM ai_agenti WHERE aktivno = 1")->fetchAll(PDO::FETCH_ASSOC);
...
<head>
    <meta charset="UTF-8">
    <title>🤖 DeepSeek AI Center – Multi-Agent System</title>
    <style>
...
<div class="container">
    ...
    <div class="header">
        <h1>🤖 DeepSeek AI Center</h1>
        <p>Multi-Agent sistem | Več API ključev | Pogovori med agenti | Zgodovina</p>
    </div>
    ...
    <div class="grid-2">
                ...
                <div class="form-group">
                    ...
                    <select name="agent_id">
                        <?php foreach ($agenti as $a): ?>
                            <option value="<?= $a['id'] ?>"><?= $a['ime'] ?> - <?= $a['opis'] ?></option>
                        <?php endforeach; ?>
                    </select>
                </div>
                <div class="form-group">
                    <label>API ključ (če pustiš prazno, uporabi privzetega)</label>
        ...
        <!-- Kartica 2: Pogovor med dvema agentoma -->
        <div class="card">
            ...
            <form method="post">
                ...
                <div class="form-group">
                    ...
                    <select name="agent1">
                        <?php foreach ($agenti as $a): ?>
                            <option value="<?= $a['id'] ?>"><?= $a['ime'] ?></option>
                        <?php endforeach; ?>
                    </select>
                </div>
                <div class="form-group">
            ...
            <?php foreach ($apiKljuci as $k): ?>
                <div style="background: #f5f5f5; padding: 8px; margin-bottom: 8px; border-radius: 8px; display: flex; justify-content: space-between;">
                    <div>
                        ...
                        <small><?= $k['model'] ?> | Poraba danes: <?= $k['porabljeno_danes'] ?> tokenov</small>
                    </div>
    ...
    <!-- REZULTATI -->
    <?php if (isset($rezultat) && $rezultat): ?>
        ...
        <div style="display: flex; gap: 10px; margin-bottom: 20px; flex-wrap: wrap;">
            <?php foreach ($sesije as $s): ?>
                <form method="post" style="display: inline;">
                    ...
                    <button type="submit" class="btn btn-danger btn-sm" onclick="return confirm('Izbriši zgodovino?')">
            ...
            <?php foreach ($zgodovina as $msg): ?>
                <div class="history-item">
                    ...
                    <div class="history-message history-<?= $msg['vloga'] ?>">
                        <?= nl2br(htmlspecialchars(substr($msg['vsebina'], 0, 500))) ?>
                        <?php if (strlen($msg['vsebina']) > 500): ?>...<?php endif; ?>
                    </div>
                </div>
            <?php endforeach; ?>
        <?php endif; ?>
